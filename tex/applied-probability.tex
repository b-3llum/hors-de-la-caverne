...
\textbf{PRB-01.} \textbf{Problème.} On lance trois dés équilibrés. Les sommes 9 et 10 s'écrivent
...
\textbf{PRB-02.} \textbf{Problème.} Le chevalier de Méré gagnait de l'argent en pariant qu'il
...
\begin{enumerate}
\item[(a)] Démontrez que le premier pari est favorable :
$ 1 - (5/6)^4 > 1/2 $.
\item[(b)] Démontrez que le second ne l'est pas :
...
\item[(c)] Mettez le doigt sur l'étape fausse du
raisonnement de proportionnalité.
\item[(d)] Le problème des partis : deux joueurs misent la même somme sur un jeu de pile ou face
...
\textbf{PRB-03.} \textbf{Problème.} On suppose les anniversaires indépendants et uniformes sur
365 jours.
\begin{enumerate}
\item[(a)] Établissez la probabilité que $ n $ personnes aient toutes des anniversaires distincts :
...
qu'improbable.
\item[(b)] Expliquez maintenant le phénomène, et pas seulement le nombre : avec $ N $ anniversaires
...
\textbf{PRB-04.} \textbf{Problème.} Une voiture est cachée uniformément au hasard derrière
...
\begin{enumerate}
\item[(a)] Calculez, à partir de la
...
probabilité $ 2/3 $.
\item[(b)] Changez maintenant une règle : le présentateur ignore où est la voiture, ouvre uniformément
...
\item[(c)] Dites en une phrase ce que ce couple de
...
soutenant qu'elle avait tort. Paul Erdős serait resté incrédule jusqu'à ce qu'on lui montre une
...
\textbf{PRB-05.} \textbf{Problème.} On lance indéfiniment une pièce équilibrée, les lancers étant indépendants.
\begin{enumerate}
\item[(a)] Démontrez, à partir de la définition de l'indépendance, qu'après n'importe quelle suite
...
\item[(b)] Ceux qui fabriquent de fausses suites \og{} aléatoires \fg{} évitent les longues séries, et c'est
...
six et majorez la probabilité qu'aucun bloc ne soit entièrement \og{} face \fg{}.) (iii) Dites
...
\textbf{PRB-06.} \textbf{Problème.} Une maladie touche 1 personne sur 1000. Un test la
...
\begin{enumerate}
\item[(a)] Démontrez le théorème de Bayes
...
dénominateur.
\item[(b)] Calculez $ P(\text{maladie} \mid \text{test positif}) $ exactement, et expliquez avec des
...
\item[(c)] Recalculez en supposant que la personne a été testée à cause de symptômes qui portent la
...
\textbf{PRB-23.} \textbf{Problème.} Une famille a deux enfants, chacun étant indépendamment
...
\textbf{PRB-24.} \textbf{Problème.} Quatre dés équilibrés à six faces portent les valeurs
...
\textbf{PRB-25.} \textbf{Problème.} À une soirée, $ n $ invités déposent leur chapeau au vestiaire, et un
...
\begin{enumerate}
\item[(a)] Par inclusion-exclusion, démontrez que la probabilité qu'il n'y ait aucune rencontre vaut
\[ p_n = \sum_{k=0}^{n} \frac{(-1)^k}{k!} , \]
de sorte que le nombre de permutations sans point fixe (un \textit{dérangement}) vaut
$ D_n = n!\, p_n $.
\item[(b)] Déduisez-en que $ p_n \to e^{-1} $ et que $ |p_n - e^{-1}| < \frac{1}{(n+1)!} $ --- la
réponse est donc essentiellement la même pour 10 invités et pour 10 millions.
\item[(c)] Démontrez la récurrence $ D_n = (n-1)\big( D_{n-1} + D_{n-2} \big) $ pour $ n \ge 3 $ par
un argument de dénombrement, et non par un calcul algébrique sur la formule.
\item[(d)] Démontrez que la probabilité d'avoir exactement $ m $ rencontres vaut
...
\textbf{PRB-35.} \textbf{Problème.} Les axiomes de Kolmogorov disent seulement ceci : tout
...
\textbf{PRB-36.} \textbf{Problème.} Lors d'une élection, le candidat A recueille $ p $ voix et le candidat B en
...
\textbf{PRB-07.} \textbf{Problème.}
\begin{enumerate}
\item[(a)] Démontrez que $ E[X+Y] = E[X] + E[Y] $ pour deux variables aléatoires quelconques
...
pour démontrer :
\item[(b)] le théorème du collectionneur de vignettes --- en tirant uniformément avec remise parmi $ n $
...
\item[(c)] une permutation uniformément aléatoire de $ n $ lettres a exactement $ 1 $ point fixe en
moyenne, et cela pour tout $ n $ ;
\item[(d)] défi : son nombre moyen de cycles vaut $ H_n $.
\end{enumerate}
...
qu'il serait sans espoir de traiter conjointement deviennent triviales une par une, et Erdős a
...
\textbf{PRB-08.} \textbf{Problème.}
\begin{enumerate}
\item[(a)] Démontrez l'inégalité de Markov : pour $ X\ge 0 $
et $ a>0 $, $ P(X\ge a) \le E[X]/a $.
\item[(b)] Déduisez-en l'inégalité de Bienaymé-Tchebychev
$ P(|X-\mu| \ge k\sigma) \le 1/k^2 $.
\item[(c)] Démontrez que la variance d'une somme de variables aléatoires deux à deux indépendantes est la
...
\item[(d)] Appliquez-la : combien de lancers d'une pièce équilibrée Tchebychev exige-t-il pour garantir,
...
\textbf{PRB-09.} \textbf{Problème.} Des épreuves indépendantes réussissent avec probabilité $ p $.
\begin{enumerate}
\item[(a)] Établissez la loi géométrique $ P(T = k) = (1-p)^{k-1}p $ de l'instant du premier succès et
vérifiez qu'elle somme à 1.
\item[(b)] Démontrez $ E[T] = 1/p $ de deux façons : en sommant la série, et par l'identité de
...
\item[(c)] Démontrez que la loi géométrique est sans mémoire ---
$ P(T > m+n \mid T > m) = P(T > n) $ --- et démontrez que c'est la \textit{seule} loi sur
$ \{1,2,3,\dots\} $ ayant cette propriété.
\item[(d)] Établissez la loi binomiale négative de l'instant du $ r $-ième succès, présentez-la comme
...
\textbf{PRB-10.} \textbf{Problème.} Un joueur part avec $ i $ unités et mise une unité par
...
\begin{enumerate}
\item[(a)] Écrivez l'équation aux différences vérifiée par la probabilité de ruine $ r_i $,
...
\item[(b)] Calculez la durée moyenne de la partie.
\item[(c)] Démontrez que contre un adversaire infiniment riche ($ N \to \infty $) à jeu équitable, la
ruine est certaine.
\item[(d)] Chiffrez l'avantage de la maison : avec $ p = 18/38 $ (roulette américaine, mise sur le
...
\textbf{PRB-11.} \textbf{Problème.} On lance une pièce équilibrée jusqu'au premier \og{} face \fg{} ;
si cela se produit au lancer $ k $, le casino verse $ 2^k $ ducats.
\begin{enumerate}
\item[(a)] Démontrez que le gain espéré est infini --- et pourtant personne ne paierait 1000 ducats pour
jouer une fois.
\item[(b)] La résolution de Daniel Bernoulli : un joueur de fortune $ w $ évalue l'argent par
...
\item[(c)] La résolution de Feller : l'espérance est simplement le mauvais résumé pour un jeu unique à
...
\item[(d)] Expliquez pourquoi (i)--(iii) ne se contredisent pas.
\end{enumerate}
...
\textbf{PRB-12.} \textbf{Problème.} On laisse tomber une aiguille de longueur $ \ell $ sur un
...
\textbf{PRB-13.} \textbf{Problème.} La marche aléatoire simple sur $ \mathbb{Z}^d $ saute
...
\begin{enumerate}
\item[(a)] Démontrez le critère : la marche est récurrente si et seulement si
$ \sum_n P(S_{2n} = 0) = \infty $.
\item[(b)] Pour $ d=1 $, montrez que
$ P(S_{2n}=0) = \binom{2n}{n}2^{-2n} \sim \frac{1}{\sqrt{\pi n}} $ (Stirling, ou PRB-14), et
concluez à la récurrence.
\item[(c)] Pour $ d=2 $, démontrez que $ P(S_{2n}=0) \sim \frac{c}{n} $ (une identité combinatoire
ramène le calcul à deux marches unidimensionnelles indépendantes) --- récurrence de nouveau.
\item[(d)] Pour $ d=3 $, montrez que $ P(S_{2n}=0) = O(n^{-3/2}) $ et concluez à la transience : avec
...
\item Lecture : Rick Durrett, \textit{Probability: Theory and Examples} (PDF gratuit, Duke (\url{https://sites.math.duke.edu/\textasciitilde{}rtd/PTE/PTE5_011119.pdf}))
\end{refs}

\bigskip

\begin{problem}[{La courbe en cloche gagne sa place : de Moivre-Laplace --- Licence}]
\textbf{PRB-14.} \textbf{Problème.} Soit $ S_n $ le nombre de \og{} face \fg{} en $ n $ lancers
d'une pièce équilibrée.
\begin{enumerate}
\item[(a)] Démontrez une formule de Stirling faible $ n! \sim C\sqrt{n}\,(n/e)^n $ en comparant
...
\item[(b)] Démontrez le théorème limite local : pour $ k = n/2 + x\sqrt{n}/2 $ avec $ x $ borné,
\[ P(S_n = k) \sim \frac{2}{\sqrt{2\pi n}}\; e^{-x^2/2} . \]
(iii) Sommez : déduisez-en le théorème de Moivre-Laplace
$ P\big( a \le \frac{S_n - n/2}{\sqrt{n}/2} \le b \big) \to
\frac{1}{\sqrt{2\pi}}\int_a^b e^{-x^2/2}\,dx $. (iv) Énoncez soigneusement --- sans démonstration ---
...
\item Lecture : Rick Durrett, \textit{Probability: Theory and Examples}, ch. 3 (PDF gratuit, Duke (\url{https://sites.math.duke.edu/\textasciitilde{}rtd/PTE/PTE5_011119.pdf}))
\item Cours : MIT OCW 18.600 Probability and Random Variables (\url{https://ocw.mit.edu/courses/18-600-probability-and-random-variables-fall-2019/})
\end{refs}
...
\textbf{PRB-26.} \textbf{Problème.} Soit $ T $ une variable aléatoire telle que
...
\textbf{PRB-27.} \textbf{Problème.} Soient $ U_1, \dots, U_n $ indépendantes et uniformes sur
...
\textbf{PRB-28.} \textbf{Problème.} Soient $ X_1, \dots, X_n $ indépendantes avec $ X_i \sim
\mathrm{Bernoulli}(p_i) $, soit $ S_n = X_1 + \cdots + X_n $, et posons
$ \lambda = p_1 + \cdots + p_n $.
\begin{enumerate}
\item[(a)] Démontrez le théorème limite de Poisson sous sa forme classique : si $ p_i = \lambda/n $
pour tout $ i $, alors pour chaque $ k $ fixé
\[ P(S_n = k) \longrightarrow \frac{e^{-\lambda}\lambda^k}{k!} \qquad (n \to \infty) . \]
\item[(b)] Démontrez qu'une variable de Poisson($ \lambda $) a une espérance et une variance toutes
...
des paramètres.
\item[(c)] Renforcez la limite en une inégalité --- le théorème de Le Cam : la distance en variation totale
...
\item[(d)] Testez la théorie sur le jeu de données fondateur. Ladislaus von Bortkiewicz a relevé, pour 10
...
\textbf{PRB-37.} \textbf{Problème.} Soient $ X_1, X_2, \dots, X_n $ indépendantes de densité de
...
\textbf{PRB-38.} \textbf{Problème.} Soient $ S_0 = 0 $ et $ S_k = X_1 + \cdots + X_k $, où les $ X_i $ sont
...
\textbf{PRB-15.} \textbf{Problème.} Pour des événements $ A_1, A_2, \dots $, on écrit
$ \{A_n \text{ i.s.}\} = \bigcap_m \bigcup_{n\ge m} A_n $ (\og{} $ A_n $ infiniment souvent \fg{}).
\begin{enumerate}
\item[(a)] Démontrez le premier lemme de Borel-Cantelli : $ \sum P(A_n) < \infty $ entraîne
$ P(A_n \text{ i.s.}) = 0 $.
\item[(b)] Démontrez le second : si les $ A_n $ sont indépendants et $ \sum P(A_n) = \infty $, alors
$ P(A_n \text{ i.s.}) = 1 $ ; exhibez un contre-exemple montrant qu'on ne peut pas simplement
supprimer l'indépendance.
\item[(c)] Application --- le singe à la machine à écrire : des lettres aléatoires i.i.d. uniformes
contiennent presque sûrement \og{} ABRACADABRA \fg{} une infinité de fois.
\item[(d)] Application --- les plus longues séries : pour des lancers de pièce équilibrée, démontrez que la
...
trancher. La partie (iv) est la loi d'Erdős-Rényi sous sa forme la plus simple --- la version
...
\textbf{PRB-16.} \textbf{Problème.}
\begin{enumerate}
\item[(a)] Définissez une martingale et un temps d'arrêt ; énoncez le théorème d'arrêt optionnel, et
démontrez-le pour les temps d'arrêt bornés.
\item[(b)] Un singe tape des majuscules i.i.d. uniformes. À chaque frappe, un nouveau joueur arrive, mise
...
\item[(c)] Déduisez-en la version pile ou face : le temps d'attente moyen vaut 10 pour FPF mais 8 pour
...
\item[(d)] Vérifiez les hypothèses du théorème d'arrêt optionnel dans votre argument --- c'est là que
...
\textbf{PRB-17.} \textbf{Problème.} Dans la percolation par arêtes sur $ \mathbb{Z}^2 $,
...
$ p_c = \sup\{p : \theta(p) = 0\} $.
\begin{enumerate}
\item[(a)] Démontrez que $ \theta $ est croissante en $ p $ (construisez tous les processus de
percolation sur un même espace probabilisé).
\item[(b)] Démontrez $ p_c \ge 1/3 $ : dénombrez les chemins auto-évitants issus de l'origine --- au plus
$ 4\cdot 3^{n-1} $ de longueur $ n $ --- et majorez la probabilité qu'un long chemin soit
ouvert.
\item[(c)] Démontrez $ p_c \le 2/3 $ par l'argument de contour de Peierls : un amas fini est entouré
d'un circuit d'arêtes duales fermées ; dénombrez les circuits duaux et majorez leur probabilité
totale.
\item[(d)] Concluez que $ 0 < p_c < 1 $ : une véritable transition de phase. Énoncez --- avec
...
\textbf{PRB-18.} \textbf{Problème.} Une suite $ X_1, X_2, \dots $ est
\textit{échangeable} si sa loi est invariante par toute permutation d'un nombre fini d'indices.
\begin{enumerate}
\item[(a)] Montrez que toute suite i.i.d. est échangeable, et trouvez une suite échangeable qui n'est pas
i.i.d.
\item[(b)] Démontrez le théorème de de Finetti pour les suites à valeurs dans $ \{0,1\} $ : une suite
...
(iii) Montrez que l'infinité est essentielle : exhibez une famille échangeable finie --- par exemple
...
\textbf{PRB-29.} \textbf{Problème.} Soient $ X_1, X_2, \dots $ i.i.d. avec
...
\textbf{PRB-30.} \textbf{Problème.} Soit $ S_n $ le nombre de \og{} face \fg{} en $ n $ lancers d'une
...
\textbf{PRB-31.} \textbf{Problème.} Dans un processus de Galton-Watson, $ Z_0 = 1 $ et chaque individu de la
...
$ p_1 < 1 $.
\begin{enumerate}
\item[(a)] Démontrez que la fonction génératrice de $ Z_n $ est la composée $ n $-ième
$ f_n = f \circ f \circ \cdots \circ f $, et déduisez-en $ E[Z_n] = m^n $.
\item[(b)] Soit $ q = P(Z_n = 0 \text{ pour un certain } n) $ la probabilité d'extinction. Démontrez que
...
\item[(c)] Démontrez que $ W_n = Z_n / m^n $ est une martingale positive, donc converge presque
...
\item[(d)] Calculez $ q $ explicitement lorsque la loi de reproduction est Poisson($ m $) puis
...
\textbf{PRB-39.} \textbf{Problème.} Soient $ (\Omega, \mathcal{F}, P) $ un espace probabilisé,
...
\item Lecture : Rick Durrett, \textit{Probability: Theory and Examples}, ch. 4 (PDF gratuit, Duke (\url{https://sites.math.duke.edu/\textasciitilde{}rtd/PTE/PTE5_011119.pdf}))
\end{refs}

\bigskip

\begin{problem}[{Fonctions caractéristiques et théorème de convergence de Lévy --- Master}]
\textbf{PRB-40.} \textbf{Problème.} Pour une variable aléatoire réelle $ X $, on définit sa fonction
...
\textbf{PRB-41.} \textbf{Problème.} Soient $ X_1, X_2, \dots $ i.i.d. telles que
...
\textbf{PRB-42.} \textbf{Problème.} Un mouvement brownien standard sur $ [0,1] $ est un processus
...
\textbf{PRB-19.} \textbf{Problème.} Soit $ c_n $ le nombre de chemins auto-évitants de
$ n $ pas issus de l'origine dans un réseau.
\begin{enumerate}
\item[(a)] Démontrez --- cette partie est entièrement à votre portée --- que $ c_{m+n} \le c_m c_n $, et
...
\item[(b)] Démontrez les majorations et minorations grossières $ 2 \le \mu \le 3 $ pour
$ \mathbb{Z}^2 $.
\item[(c)] \textbf{Résolu (2012) :} sur le réseau en nid d'abeilles,
...
\item[(d)] \textbf{Ouvert :} la valeur exacte de $ \mu $ pour $ \mathbb{Z}^2 $ (numériquement
...
\textbf{PRB-20.} \textbf{Problème.} Poursuivez à partir de PRB-17.
\begin{enumerate}
\item[(a)] Dans le plan, le tableau critique est un théorème : pour la percolation par sites sur le
...
\item[(b)] En grande dimension ($ d \ge 11 $, Fitzner-van der Hofstad 2017, après Hara-Slade pour
$ d \ge 19 $), le développement en lacets donne les exposants de champ moyen.
\item[(c)] \textbf{Ouvert :} pour $ 3 \le d \le 10 $ --- y compris la dimension physique 3 --- il n'est pas
...
\textbf{PRB-21.} \textbf{Problème.} De nombreux modèles de croissance unidimensionnels ---
...
\begin{enumerate}
\item[(a)] Parties accessibles : dérivez heuristiquement le changement d'échelle 1:2:3 à partir de
...
\item[(b)] \textbf{Résolu pour les modèles exactement solubles :} le processus de Markov limite --- le point
...
\item[(c)] \textbf{Ouvert :} l'universalité --- le fait que les modèles génériques de la classe (lois de poids
...
\textbf{PRB-22.} \textbf{Problème.} L'ensemble de boucles conforme CLE$\kappa$
...
réseau.
\begin{enumerate}
\item[(a)] Parties accessibles : précisez ce que signifie \og{} limite d'échelle conformément invariante \fg{}
...
\item[(b)] \textbf{Cas résolus, avec attribution :} les interfaces de la percolation critique par sites
...
\item[(c)] \textbf{Ouvert :} la convergence vers CLE$\kappa$ pour la grande majorité des modèles
...
\textbf{PRB-32.} \textbf{Problème.} Le verre de spin de Sherrington-Kirkpatrick attribue à chaque
...
\textbf{PRB-33.} \textbf{Problème.} Une formule $ k $-SAT aléatoire tire $ m = \alpha n $
...
\textbf{PRB-34.} \textbf{Problème.} Pour une chaîne de Markov irréductible apériodique sur un espace d'états fini,
...
\begin{enumerate}
\item[(a)] Partie accessible : démontrez que $ d $ est décroissante et que
...
\item[(b)] Démontrez une coupure de bout en bout pour la marche aléatoire paresseuse sur l'hypercube
...
couplage.
\item[(c)] Comprenez le cas célèbre : Bayer et Diaconis (1992) ont démontré que le battage américain
...
\item[(d)] \textbf{Ouvert :} Aldous et Diaconis ont demandé dans les années 1980 un critère général de
...
\textbf{PRB-43.} \textbf{Problème.} L'inégalité de corrélation gaussienne affirme que pour une
...
rendu précis de l'état des lieux : quels cas particuliers étaient connus (l'inégalité de Šidák de
...
\item Article : Z. Šidák, \og{} Rectangular confidence regions for the means of multivariate normal distributions \fg{}, J. Amer. Statist. Assoc. 62 (1967), 626--633
\end{refs}

\bigskip

\begin{problem}[{À quelle vitesse le théorème central limite converge-t-il ? --- Recherche}]
\textbf{PRB-44.} \textbf{Problème.} Soient $ X_1, \dots, X_n $ i.i.d. d'espérance $ 0 $, de variance
