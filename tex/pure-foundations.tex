...
\textbf{FND-06.} On dit qu'un ensemble est \textit{dénombrable} s'il est fini ou en
...
\item Article : Georg Cantor, \og{} Über eine Eigenschaft des Inbegriffes aller reellen algebraischen Zahlen \fg{} (1874), Crelle's Journal
\end{refs}

\bigskip

\begin{problem}[{Diagonalisation : $\mathbb{R}$ n'est pas dénombrable --- Licence}]
\textbf{FND-07.} \begin{enumerate}
...
\textbf{FND-08.} Supposons qu'il existe des injections $ f\colon A \to B $ et
$ g\colon B \to A $.
\begin{enumerate}
...
\textbf{FND-09.} En travaillant dans ZF, démontrez que les énoncés suivants sont équivalents :
\begin{enumerate}
\item[(i)] l'\textit{axiome du choix} --- toute famille d'ensembles non vides admet une fonction de choix ;
\item[(ii)] le \textit{lemme de Zorn} --- tout ensemble ordonné non vide dans lequel toute chaîne admet un
majorant contient un élément maximal ;
\item[(iii)] le \textit{théorème de Zermelo} --- tout ensemble peut
...
\textbf{FND-10.} L'addition, la multiplication et l'exponentiation ordinales se
...
\textbf{FND-11.} Écrivez un entier strictement positif en \textit{notation héréditaire en base
...
\item Article : L. Kirby and J. Paris, \og{} Accessible independence results for Peano arithmetic \fg{} (1982), Bull. London Math. Soc. 14
\item Article : R. L. Goodstein, \og{} On the restricted ordinal theorem \fg{} (1944), J. Symbolic Logic 9
\end{refs}

\bigskip

\begin{problem}[{La compacité en miniature --- Licence}]
\textbf{FND-12.} \begin{enumerate}
...
théorème de De Bruijn--Erd\H{o}s : pour tout $ k $, un graphe (éventuellement infini) est
...
compacité du premier ordre de FND-13, et l'application au coloriage est due à De Bruijn et Erd\H{o}s
...
dénombrable, le lemme de l'infini de K\H{o}nig (1927) --- tout arbre infini à branchement fini possède
...
\item Contexte : Lemme de K\H{o}nig --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Lemme_de_K%C5%91nig})
\item Contexte : Théorème de De Bruijn--Erd\H{o}s (théorie des graphes) --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Th%C3%A9or%C3%A8me_de_De_Bruijn-Erd%C5%91s_(th%C3%A9orie_des_graphes)})
\end{refs}

\bigskip

\begin{problem}[{Pas de descente infinie dans les ordinaux --- Licence, short}]
\textbf{FND-26.} Démontrez qu'il n'existe pas de suite infinie d'ordinaux
...
\textbf{FND-27.} Coloriez chaque partie à deux éléments de $\mathbb{N}$ avec l'une d'un
...
\item Article : F. P. Ramsey, \og{} On a problem of formal logic \fg{} (1930), Proc. London Math. Soc. 30
\end{refs}

\bigskip

\begin{problem}[{Ultrafiltres, et des limites qui existent toujours --- Licence}]
\textbf{FND-28.} Un \textit{filtre} sur un ensemble $ S $ est une famille non vide $ F $ de
...
existe exactement un $ L \in \mathbb{R} $ tel que $ \{ n : |x_n - L| < \varepsilon \} \in U $
pour tout $ \varepsilon > 0 $. Démontrez que cette $ U $-limite est linéaire et monotone en la
...
compactifié de Stone--\v{C}ech, les ultraproduits et le théorème de \L{}o\'s, l'analyse non standard de
...
\textbf{FND-38.} En logique du second ordre avec la sémantique \textit{standard}, un
...
\textbf{FND-39.} Une partie $ A \subseteq \mathbb{N} $ est $ \Sigma^0_n $ si elle est
...
\textbf{FND-40.} On dit qu'un réel $ \alpha $ est \textit{calculable} si un
...
\item Article : A. M. Turing, \og{} On computable numbers, with an application to the Entscheidungsproblem \fg{} (1936), \textit{Proceedings of the London Mathematical Society} 42
\end{refs}
...
\textbf{FND-14.} Soit $ \mathrm{Th}(\mathbb{N}) $ l'ensemble de tous les
...
\textbf{FND-16.} Soit $ T $ une théorie cohérente, récursivement axiomatisée,
...
\textbf{FND-17.} Soit $ T $ comme en FND-16, et soit
...
\item Article : M. H. Löb, \og{} Solution of a problem of Leon Henkin \fg{} (1955), J. Symbolic Logic 20
...
\textbf{FND-18.} L'hypothèse du continu (HC) affirme que toute partie infinie
...
\item Article : Paul Cohen, \og{} The independence of the continuum hypothesis \fg{} (1963), Proc. Nat. Acad. Sci. USA 50
\end{refs}

\bigskip

\begin{problem}[{Un îlot de décidabilité : l'arithmétique de Presburger --- Master}]
\textbf{FND-19.} L'arithmétique de Presburger est la théorie du premier ordre de
...
\noindent Moj\.zesz Presburger a démontré la décidabilité en 1929, alors qu'il était
...
\item Lecture : George Boolos, John Burgess, and Richard Jeffrey, \textit{Computability and Logic}
\item Article : M. J. Fischer and M. O. Rabin, \og{} Super-exponential complexity of Presburger arithmetic \fg{} (1974), SIAM--AMS Proceedings 7
\end{refs}

\bigskip

\begin{problem}[{Le principe de réflexion --- Master, short}]
\textbf{FND-29.} Soit $ V_\alpha $ la hiérarchie cumulative
...
\textbf{FND-30.} Démontrez que $\mathbb{Q}$ est une partie $ F_\sigma $ de $\mathbb{R}$ --- une
...
\textbf{FND-31.} On dit que $ S \subseteq \mathbb{N}^k $ est \textit{diophantien} s'il existe un
...
\item Article : Martin Davis, \og{} Hilbert's tenth problem is unsolvable \fg{} (1973), \textit{American Mathematical Monthly} 80
\end{refs}

\bigskip

\begin{problem}[{Construire une extension générique à la main --- Master}]
\textbf{FND-32.} FND-18 vous demandait de dire ce que réalise le forcing. Celui-ci vous demande
...
Solovay et Vop\v{e}nka a été inventée précisément pour rendre le même contenu inévitable ; lire les
...
\item Article : Timothy Y. Chow, \og{} A beginner's guide to forcing \fg{} (2008), arXiv:0712.1320 (\url{https://arxiv.org/abs/0712.1320})
\end{refs}

\bigskip

\begin{problem}[{Cofinalité et théorème de König --- Master}]
\textbf{FND-41.} La \textit{cofinalité} $ \operatorname{cf}(\alpha) $ d'un ordinal limite
...
\textbf{FND-42.} Cantor a caractérisé $\mathbb{R}$ à isomorphisme d'ordre près comme l'unique ordre total
...
\item Article : R. M. Solovay and S. Tennenbaum, \og{} Iterated Cohen extensions and Souslin's problem \fg{}, \textit{Annals of Mathematics} 94 (1971), 201--245
\end{refs}

\bigskip

\begin{problem}[{Interpolation et définissabilité --- Master, short}]
\textbf{FND-43.} Énoncez précisément le théorème d'interpolation de Craig pour
...
\textbf{FND-20.} Un cardinal $ \kappa $ est \textit{inaccessible} s'il est
...
\textbf{FND-21.} Le $ L $ de Gödel (FND-18) est un univers intérieur canonique et ordonné,
...
\item Article : W. H. Woodin, \og{} In search of Ultimate-L: the 19th Midrasha Mathematicae Lectures \fg{} (2017), Bull. Symbolic Logic 23
\end{refs}

\bigskip

\begin{problem}[{La frontière de la décidabilité : le problème de la fonction exponentielle de Tarski --- Recherche}]
\textbf{FND-22.} Tarski a démontré (1951) que la théorie du premier ordre du
...
\item Article : A. Macintyre and A. J. Wilkie, \og{} On the decidability of the real exponential field \fg{} (1996), in \textit{Kreiseliana: About and Around Georg Kreisel}
\end{refs}

\bigskip

\begin{problem}[{p = t : un problème de soixante-dix ans résolu --- Recherche}]
\textbf{FND-23.} Pour des ensembles infinis $ A, B \subseteq \mathbb{N} $, on écrit
...
\item Article : M. Malliaris and S. Shelah, \og{} General topology meets model theory, on p and t \fg{} (2013), Proc. Nat. Acad. Sci. USA 110
\item Article : M. Malliaris and S. Shelah, \og{} Cofinality spectrum theorems in model theory, set theory, and general topology \fg{} (2016), J. Amer. Math. Soc. 29
\end{refs}

\bigskip

\begin{problem}[{Le dixième problème de Hilbert sur les rationnels --- Recherche, short}]
\textbf{FND-33.} (Ouvert.) Sur les entiers, il n'existe aucun algorithme
...
\begin{refs}
\item Contexte : Dixième problème de Hilbert --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Dixi%C3%A8me_probl%C3%A8me_de_Hilbert})
\item Article : J. Koenigsmann, \og{} Defining Z in Q \fg{} (2016), \textit{Annals of Mathematics} 183
\item Article : P. Koymans and C. Pagano, \og{} Hilbert's tenth problem via additive combinatorics \fg{} (2024), arXiv:2412.01768 (\url{https://arxiv.org/abs/2412.01768})
...
\textbf{FND-34.} (Ouvert.) Supposons $ \aleph_\omega $ cardinal limite fort,
c'est-à-dire $ 2^{\aleph_n} < \aleph_\omega $ pour tout $ n < \omega $ ; Shelah a démontré dans
ZFC qu'alors $ 2^{\aleph_\omega} < \aleph_{\omega_4} $. Déterminez s'il est cohérent
avec ZFC que, sous la même hypothèse, $ 2^{\aleph_\omega} > \aleph_{\omega_1} $ --- de façon
...
\textbf{FND-35.} (Partiellement résolu ; la question centrale est ouverte.) Le théorème de
...
Stone--\v{C}ech. Blass, Hirst et Simpson l'ont coincé entre $ \mathrm{ACA}_0 $ et
...
\item Article : N. Hindman, \og{} Finite sums from sequences within cells of a partition of N \fg{} (1974), \textit{Journal of Combinatorial Theory, Series A} 17
\end{refs}

\bigskip

\begin{problem}[{Les Cinq Grands, et pourquoi ils sont cinq --- Recherche}]
\textbf{FND-44.} Les mathématiques à rebours demandent, d'un théorème des mathématiques
...
\item Lecture : Stephen G. Simpson, \textit{Subsystems of Second Order Arithmetic}
\item Article : A. Montalbán, \og{} Open questions in reverse mathematics \fg{}, \textit{Bulletin of Symbolic Logic} 17 (2011), 431--454
\end{refs}

\bigskip

\begin{problem}[{Quelle est la force de l'axiome du forcing propre ? --- Recherche, short}]
\textbf{FND-45.} (\textbf{Ouvert.}) L'axiome du forcing propre PFA étend
...
\item Contexte : Grand cardinal --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Grand_cardinal})
\item Article : J. T. Moore, \og{} The proper forcing axiom \fg{}, \textit{Proceedings of the International Congress of Mathematicians} (Hyderabad, 2010)
\end{refs}
