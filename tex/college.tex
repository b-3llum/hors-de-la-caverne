...
\textbf{NT-54.} Tout entier à quatre chiffres s'écrit $ n = 1000a + 100b + 10c + d $, où
...
\textbf{NT-55.} Écrivez les entiers de $ 2 $ à $ 100 $. Barrez les multiples de $ 2 $ autres
...
\textbf{NT-56.} Démontrez que la somme de deux nombres impairs est toujours
...
\textbf{NT-57.} On veut calculer $ 1 + 2 + 3 + \cdots + n $ sans poser l'addition.
\begin{enumerate}
...
\textbf{NT-58.} Soient $ a $ et $ b $ deux entiers strictement positifs avec
...
\textbf{NT-59.} Un phare s'allume toutes les $ 12 $ secondes, un autre toutes
...
\textbf{NT-60.} Démontrez que le dernier chiffre de $ 2^n $, pour
...
\textbf{NT-61.} Un entier strictement positif est dit \textit{parfait} lorsqu'il est égal à la somme
...
\textbf{NT-62.} L'intuition dit qu'il y a \og{} deux fois moins \fg{} de nombres pairs que d'entiers,
...
\textbf{NT-63.} Les scribes égyptiens n'écrivaient que des fractions de numérateur $ 1 $, et
...
\textbf{NT-64.} On pose $ F_1 = 1 $, $ F_2 = 1 $, et chaque terme suivant est la somme des
...
\textbf{NT-65.} Démontrez qu'un entier à quatre chiffres
...
\textbf{GEO-41.} Soit $ ABC $ un triangle. Tracez la droite passant par $ A $ et parallèle à
$ (BC) $.
\begin{enumerate}
...
\textbf{GEO-42.} Soit un triangle rectangle de côtés de l'angle droit $ a $ et $ b $, et
...
à n\oe{}uds des arpenteurs égyptiens s'en servaient pour tracer des angles droits. La démonstration par
découpage demandée ici est essentiellement celle qui accompagne le \textit{Zhoubi Suanjing} chinois, où
la figure porte le nom de \textit{xian tu}, et on la retrouve chez Bh\=askara II au XIIe siècle
...
\textbf{GEO-43.} Soient $ A $ et $ B $ deux points distincts, $ I $ le milieu de
...
\textbf{GEO-44.} On considère un cercle de rayon $ 1 $, donc de périmètre $ 2\pi $.
\begin{enumerate}
...
\textbf{GEO-45.} Un piquet vertical de $ 1{,}50 $ m projette une ombre de $ 2{,}40 $ m. Au même
...
\textbf{GEO-46.} Un patron de cube est une figure faite de six carrés reliés par des côtés, qui se
...
\textbf{GEO-47.} Une droite $ (d) $ représente la berge d'une rivière. Deux points $ A $ et
...
\textbf{GEO-48.} Soient $ A $ et $ B $ deux points d'une droite $ (d) $, et
...
\textbf{GEO-49.} On veut recouvrir le plan, sans trou ni chevauchement, avec des copies d'un même
...
\textbf{GEO-50.} Soient $ (d_1) $ et $ (d_2) $ deux droites parallèles
...
\textbf{GEO-51.} Tracez très soigneusement, sur cinq triangles quelconques et
...
\textbf{GEO-52.} Deux droites $ (d) $ et $ (d') $ sont coupées par une
...
\textbf{STA-41.} L'entreprise Duval emploie dix personnes : huit ouvriers payés 1 500 \texteuro{} par
mois, un chef d'atelier payé 2 400 \texteuro{} et le patron, qui se verse 21 600 \texteuro{}.
\begin{enumerate}
...
\textbf{STA-42.} Un journal local publie un diagramme en barres du nombre de cambriolages
...
\textbf{STA-43.} Camille veut connaître le sport préféré des 600 élèves de son collège. Un
...
\textbf{STA-44.} Deux classes de neuf élèves ont passé le même contrôle, noté sur 20. Voici les
notes obtenues.
...
\textbf{STA-45.} Au dernier contrôle de mathématiques, la classe de 4e A, qui compte
...
\textbf{STA-46.} Le foyer socio-éducatif d'un collège publie un diagramme circulaire des
...
\textit{The Visual Display of Quantitative Information}, soutient que l'\oe{}il compare mal les angles et
...
\textbf{STA-47.} Dans un collège de 800 élèves, 200 sont demi-pensionnaires ;
...
\textbf{STA-48.} Dans un collège, 40 \% des élèves sont en cycle 4 et, parmi
...
\textbf{STA-49.} Dans une classe de 27 élèves, on a relevé le nombre de frères
et s\oe{}urs de chacun : 4 élèves en ont 0, 9 en ont 1, 7 en ont 2, 4 en ont 3, 2 en ont 4 et 1 en a 6.
...
\textbf{STA-50.} Pour illustrer que le budget du club de sport a doublé, un
...
\textbf{STA-51.} On considère les notes d'un contrôle dans une classe de 25 élèves.
\begin{enumerate}
...
\textbf{STA-52.} On a interrogé 400 élèves d'un collège, dont 240 filles et 160 garçons, en
...
\begin{refs}
\item Contexte : Recherche dichotomique --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Recherche_dichotomique})
\item Voir aussi : DCS-02, la correction de la recherche dichotomique (\url{applied-discrete-cs.html#DCS-02})
\end{refs}
...
\item Voir aussi : PUZ-01 (\url{puzzles.html#PUZ-01}) ci-dessous, pour la même façon de compter ce qu'une seule observation peut trancher
\end{refs}
...
\item Voir aussi : PUZ-16 (\url{puzzles.html#PUZ-16}) ci-dessous, la variante avec des carrés parfaits, toujours ouverte
\end{refs}
...
\item Voir aussi : PUZ-59 (\url{puzzles.html#PUZ-59}) ci-dessous, où la même poussée décide du niveau d'un bassin
\end{refs}
...
\textbf{PRF-64.} Un camarade affirme : \og{} J'ai essayé avec 3, 5 et 7, ça marche à chaque
...
\textbf{PRF-65.} Un entier est pair s'il s'écrit $ 2k $ pour un certain entier k, et
...
\textbf{PRF-66.} Démontrez ou réfutez : un entier et son double n'ont
...
\textbf{PRF-67.} On part de deux nombres égaux, $ a = b $, et on
enchaîne :
...
\textbf{PRF-68.} Le principe des tiroirs dit ceci : si l'on range plus d'objets qu'on n'a
...
\textbf{PRF-69.} Dans un groupe d'au moins deux personnes, où l'amitié
...
\noindent Le raisonnement par l'absurde est la man\oe{}uvre la plus étrange de la
...
\textbf{PRF-70.} Un invariant est une quantité qui ne bouge pas alors que la situation,
...
\textbf{PRF-71.} On colorie un carré $ 5 \times 5 $ comme un damier, les quatre coins
...
tour de main que Solomon Golomb a installé au c\oe{}ur d'un domaine entier lorsqu'il a inventé
...
\item Lecture : S. W. Golomb, \textit{Polyominoes} (2\textsuperscript{e} éd., 1994), ch. 1
\end{refs}

\bigskip

\begin{problem}[{La réciproque n'est pas l'énoncé --- Collège}]
\textbf{PRF-72.} La réciproque de \og{} si P alors Q \fg{} est \og{} si Q alors P \fg{}. Ce sont deux
...
\textbf{PRF-73.} Deux droites se coupent en A. Sur la première on place B, puis M, du même
...
\textbf{PRF-74.} Trouver un argument et l'écrire sont deux travaux distincts, et le second
...
\textbf{PRF-75.} Décidez, avec démonstration, si l'affirmation \og{} la
