...
\textbf{ODE-04.} Une cuve contient 100 litres d'eau pure. De la saumure contenant 0,5 kg de
...
\textbf{ODE-05.} On considère $ m y'' + c y' + k y = 0 $ avec
...
\item[(c)] Démontrer : toute solution non nulle tend vers 0 quand $ t \to \infty $ si et seulement si
...
\textbf{ODE-06.} On excite un oscillateur non amorti à la pulsation $ \omega $ :
...
\textbf{ODE-07.} On considère le problème de Cauchy $ y' = f(t, y) $,
...
\textbf{ODE-08.} On considère $ y' = y^{2/3} $ avec $ y(0) = 0 $.
\begin{enumerate}
...
\textbf{ODE-09.} On considère le système linéaire plan $ x' = A x $ pour une matrice réelle
$ 2 \times 2 $ $ A $ avec $ \det A \ne 0 $.
\begin{enumerate}
\item[(a)] Résoudre le système dans chaque cas de valeurs propres (réelles distinctes, complexes
conjuguées, double) et esquisser les portraits de phase : col, n\oe{}uds, foyers, centres.
...
\textbf{ODE-10.} Soit $ x^* $ un point d'équilibre du système non linéaire
...
positions basses sont des foyers ou des n\oe{}uds stables et que les positions renversées sont des
cols.
...
\textbf{ODE-11.} Le système proie--prédateur de Lotka et Volterra, pour des
...
\textbf{ODE-12.} Le pendule non amorti : $ \theta'' + \frac{g}{L} \sin\theta = 0 $, lâché
...
\textbf{ODE-25.} Soient $ y_1, y_2 $ deux solutions de
...
de Józef Hoene-Wro\'nski, dont Thomas Muir a ranimé la notation des décennies plus tard. C'est la
...
\textbf{ODE-26.} Sur un ouvert simplement connexe
...
\textbf{ODE-27.} Pour $ f $ continue par morceaux sur $ [0,\infty) $ et d'ordre
...
\textbf{ODE-37.} Pour l'équation de Legendre
...
\textbf{ODE-38.} Appliquer des méthodes numériques à l'équation test de Dahlquist
...
\textbf{ODE-13.} Soit $ x^* $ un point d'équilibre de $ x' = f(x) $ dans
...
\textbf{ODE-15.} L'équation de Van der Pol, pour $ \mu >0 $ :
\[ x'' - \mu (1 - x^2)\, x' + x = 0 . \]
...
L'équation est l'oscillateur auto-entretenu prototype --- le c\oe{}ur mathématique des stimulateurs
...
\textbf{ODE-16.} On itère $ x_{n+1} = r x_n (1 - x_n) $ sur
$ [0, 1] $, $ 0 \le r \le 4 $.
\begin{enumerate}
...
\textbf{ODE-17.} Le système de Lorenz, avec les paramètres classiques
...
\textbf{ODE-18.} Trois masses ponctuelles s'attirent mutuellement par la gravitation
newtonienne.
\begin{enumerate}
...
\textbf{ODE-28.} Soit $ D \subseteq \mathbb{R}^2 $ un ouvert simplement
...
\textbf{ODE-29.} Soient $ q_1 \le q_2 $ continues sur un intervalle
...
\textbf{ODE-30.} Un système hamiltonien sur $ \mathbb{R}^{2n} $ de hamiltonien lisse
$ H(q,p) $ s'écrit
...
\textbf{ODE-39.} Soit $ A(t) $ continue et $ T $-périodique, et soit $ \Phi(t) $ la
...
\textbf{ODE-40.} Soit $ f $ un champ de vecteurs continu sur un ouvert de
...
indice $ -1 $ tandis qu'un n\oe{}ud, un foyer ou un centre a pour indice $ +1 $, et enfin que
...
\textbf{ODE-41.} Pour $ \varepsilon > 0 $ petit, on considère le problème aux limites
...
\textbf{ODE-19.} Pour un système polynomial plan
...
\textbf{ODE-20.} Une solution du problème newtonien à $ n $ corps présente une
...
presque un siècle pour y répondre. Les constructions sont des chefs-d'\oe{}uvre de la théorie
...
\textbf{ODE-21.} La plus ancienne question de la dynamique : les planètes restent-elles à peu
...
\textbf{ODE-31.} Markus et Yamabe ont conjecturé en 1960 que si
...
\textbf{ODE-32.} Un \textit{équilibre relatif} du problème newtonien à
...
\textbf{ODE-33.} On écrit un hamiltonien quasi intégrable en variables action--angle
...
\textbf{ODE-42.} L'équation de Wright est l'équation différentielle scalaire à retard
...
\textbf{ODE-43.} Un champ de vecteurs de classe $ C^1 $ sur une variété
