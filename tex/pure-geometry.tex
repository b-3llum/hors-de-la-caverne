...
\textbf{GEO-06.} Travaillez à partir des quatre premiers postulats d'Euclide --- ou, pour être
...
\textbf{GEO-07.} Le demi-plan supérieur $ \mathbb{H} = \{(x,y) : y > 0\} $, muni de la
...
\textbf{GEO-08.} Travaillez dans le plan hyperbolique, avec le modèle du demi-plan de GEO-07,
...
\textbf{GEO-09.} Un fuseau sur une sphère est une région bornée par deux demi-grands cercles de
...
\textbf{GEO-10.} Soit une courbe plane paramétrée par l'abscisse curviligne $ s $, et soit
...
\textbf{GEO-11.} Une géodésique sur une surface est une courbe localement la
...
\textbf{GEO-12.} Un plan coupe une nappe d'un cône de révolution suivant une
...
\textbf{GEO-25.} Démontrez que toute isométrie du plan euclidien est une translation, une rotation, une réflexion ou une réflexion glissée, et que toute isométrie est composée d'au plus trois réflexions.
\end{problem}
...
\textbf{GEO-26.} Démontrez le théorème de Helly dans le plan : si une famille finie de convexes de $ \mathbb{R}^2 $ est telle que trois quelconques d'entre eux ont un point commun, alors tous ont un point commun. Montrez par un exemple que \og{} trois \fg{} ne peut pas être affaibli en \og{} deux \fg{}.
\end{problem}
...
\textbf{GEO-33.} Modélisez la Terre par une sphère de rayon $ R = 6371 $ km. Le point de
...
\textbf{GEO-34.} L'inversion de centre $ O $ et de rayon $ r $ est l'application du plan
...
\begin{problem}[{L'inégalité d'Erd\H{o}s--Mordell --- Licence, short}]
\textbf{GEO-35.} Soit $ P $ un point intérieur à un triangle $ ABC $, et
...
\noindent Paul Erd\H{o}s proposa cet énoncé comme problème dans l'\textit{American Mathematical
...
\begin{refs}
\item Contexte : Théorème d'Erd\H{o}s-Mordell --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Th%C3%A9or%C3%A8me_d%27Erd%C5%91s-Mordell})
...
\textbf{GEO-13.} (Problème de compréhension.) La courbure de Gauss $ K $
...
\textbf{GEO-14.} Admettez la formule de Gauss--Bonnet locale : pour un triangle géodésique
...
\textbf{GEO-15.} Une surface est minimale si sa courbure moyenne (la moyenne des courbures
...
\textbf{GEO-16.} Travaillez dans le plan projectif complexe $ \mathbb{P}^2(\mathbb{C}) $.
\begin{enumerate}
...
\textbf{GEO-17.} Soit $ E $ une cubique non singulière de
...
\textbf{GEO-27.} Utilisez le Theorema Egregium pour démontrer qu'aucune région d'une sphère, si petite soit-elle, ne peut être envoyée sur le plan sans déformer les distances. Dites exactement quelle quantité fait obstruction.
\end{problem}
...
\textbf{GEO-28.} Pour un polyèdre convexe, définissez le défaut angulaire en un sommet comme $ 2\pi $ moins la somme des angles des faces qui s'y rencontrent. Démontrez le théorème de Descartes selon lequel le défaut total sur tous les sommets vaut $ 4\pi $, et expliquez en quoi c'est le théorème de Gauss--Bonnet pour une surface dont la courbure est concentrée en des points.
\end{problem}
...
\textbf{GEO-36.} GEO-05 montait vers le théorème isopérimétrique et s'arrêtait là où il fallait
...
\begin{problem}[{Courbure totale, et ce que coûte un n\oe{}ud --- Master}]
\textbf{GEO-37.} Soit $ \gamma $ une courbe fermée de classe $ C^2 $ dans
...
minimale d'un type de n\oe{}ud donné reste une question vivante.
...
\textbf{GEO-38.} Dites qu'une surface lisse de $ \mathbb{R}^3 $ est
...
\textbf{GEO-19.} Avec quelle densité peut-on empiler des boules égales sans recouvrement dans
$ \mathbb{R}^n $ ?
\begin{enumerate}
...
\textbf{GEO-20.} Un \textit{einstein} (\og{} une pierre \fg{}) est une tuile unique dont les copies pavent
...
\textbf{GEO-21.} Quelle est la plus grande aire d'une forme plane que l'on puisse déplacer (par
...
\textbf{GEO-22.} Le problème de Plateau demande : toute courbe de bord fermée de l'espace
...
\textbf{GEO-29.} Dressez un tableau précis de l'empilement de sphères le plus dense connu en dimensions $ 1 $ à $ 24 $, en indiquant pour chaque dimension si l'optimalité est \textit{démontrée} ou seulement \textit{conjecturée}. Seules les dimensions $ 1, 2, 3, 8 $ et $ 24 $ sont réglées.
\end{problem}
...
\textbf{GEO-39.} (Ouvert dans presque toutes les dimensions.) Le nombre de
...
\textbf{GEO-40.} (Ouvert.) Une \textit{courbe de Jordan} est l'image d'une application continue
