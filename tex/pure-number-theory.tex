...
\textbf{NT-09.} Démontrer le théorème de Wilson : si $p$ est premier alors
...
\textbf{NT-10.} Démontrer le théorème des restes chinois : si
...
\textbf{NT-11.} Soit $\varphi(n)$ le nombre d'entiers de
...
\textbf{NT-12.} Soit $p$ un nombre premier impair. Un résidu non nul $a$
...
\textbf{NT-13.} Pour un nombre premier impair $p$ et $a$ premier avec $p$,
le symbole de Legendre $\left(\frac{a}{p}\right)$ vaut $+1$ si $a$ est un résidu
quadratique modulo $p$ et $-1$ sinon.
\begin{enumerate}
\item[(i)] Démontrer que le symbole est complètement
multiplicatif en $a$.
\item[(ii)] À l'aide du lemme de Gauss ou du critère d'Euler, calculer à la main
...
$383$.
\item[(iv)] La mériter maintenant : travailler et présenter une démonstration complète --- la
...
\textbf{NT-14.} Soit $d$ un entier strictement positif qui n'est pas un
...
pour tout $d$ en 1768 au moyen des fractions continues. Le problème des b\oe{}ufs d'Archimède, s'il est
...
\textbf{NT-15.} Démontrer que tout nombre premier $p \equiv 1 \pmod 4$ est
...
\textbf{NT-16.} Démontrer que tout entier naturel est somme de quatre carrés
...
\textbf{NT-17.} Notons $v_p(m)$ l'exposant du nombre premier
...
machines de travail : les estimations de Tchebychev et d'Erd\H{o}s sur les nombres premiers (NT-20,
...
\textbf{NT-18.} À l'aide de la série $e = \sum_{n \ge 0} 1/n!$,
...
\textbf{NT-19.} Le petit théorème de Fermat fournit un test de
composition : si $a^{n-1} \not\equiv 1 \pmod n$ pour un certain $a$ premier avec $n$, alors
$n$ est composé.
\begin{enumerate}
\item[(i)] Montrer que $2^{340} \equiv 1 \pmod{341}$ bien que
$341 = 11 \times 31$ --- la réciproque de Fermat est donc fausse --- et trouver une base qui démasque
$341$.
\item[(ii)] Démontrer le critère de Korselt : un entier composé $n$ vérifie
...
divisant $n$.
\item[(iii)] Démontrer que $561$ est un tel nombre --- un nombre de Carmichael --- et que
c'est le plus petit.
\end{enumerate}
...
\begin{problem}[{Le postulat de Bertrand, d'après Erd\H{o}s --- Licence}]
\textbf{NT-20.} Démontrer le postulat de Bertrand : pour tout $n \ge 1$
il existe un nombre premier $p$ tel que $n < p \le 2n$. Suivre la voie élémentaire d'Erd\H{o}s :
...
1919 ; et en 1932 Erd\H{o}s, âgé de dix-neuf ans, ouvrit son premier article par la démonstration
élémentaire que ce problème parcourt. NT-24 transforme la même machinerie en bornes générales de
Tchebychev sur $\pi$(x). Le distique tardif d'Erd\H{o}s fait partie du folklore : \og{} Tchebychev l'a dit, et je
...
\textbf{NT-38.} Soient $ p $ un nombre premier et $ a $ un entier non divisible par $ p $. Démontrer que l'ordre multiplicatif de $ a $ modulo $ p $ divise $ p - 1 $, et s'en servir pour montrer que tout diviseur premier de $ 2^{q} - 1 $, avec $ q $ premier, est congru à $ 1 $ modulo $ q $.
\end{problem}
...
\textbf{NT-39.} Démontrer qu'un entier $ n > 1 $ est premier si et seulement si $ (n-1)! \equiv -1 \pmod n $. Expliquer ensuite en deux phrases pourquoi ce critère de primalité parfait est inutilisable comme test de primalité en pratique.
\end{problem}
...
\textbf{NT-46.} Soit $ \mathbb{Z}[i] = \{ a + bi : a, b \in \mathbb{Z} \} $ et définissons la
...
\textbf{NT-47.} Soit $ p(n) $ le nombre de partitions de $ n $ --- le nombre de façons
...
\textbf{NT-48.} Soient $ p $ un nombre premier et $ m = \sum_i m_i p^i $
...
de Pascal réduit modulo $ 2 $ dessine le triangle de Sierpi\'nski. Le théorème de Lucas donne le
...
\textbf{NT-21.} Soit $n$ impair, $n - 1 = 2^s d$ avec $d$
...
\item[(iv)] Expliquer pourquoi les menteurs peuvent ne pas former un
...
\textbf{NT-22.} Posons $s_0 = 4$ et $s_{k+1} = s_k^2 - 2$.
...
\textbf{NT-23.} Lire \og{} PRIMES is in P \fg{} d'Agrawal, Kayal et Saxena.
\begin{enumerate}
\item[(i)] En guise d'échauffement, démontrer l'identité sur laquelle tout repose : pour
$\gcd(a, n) = 1$, $n$ est premier si et seulement si
$(X + a)^n \equiv X^n + a$ dans l'anneau de polynômes $(\mathbb{Z}/n\mathbb{Z})[X]$.
\item[(ii)] Expliquer pourquoi ce critère, testé littéralement, demande un temps exponentiel, et comment
...
\item[(iii)] Reconstruire l'argument d'\og{} introspection \fg{} qui borne le groupe engendré par les
congruences obtenues, et expliquer où la taille de $r$ intervient dans l'exposant final du temps
d'exécution.
\item[(iv)] Expliquer honnêtement pourquoi, vingt ans plus tard, personne n'utilise AKS en
pratique.
\end{enumerate}
...
\textbf{NT-24.} \begin{enumerate}
\item[(i)] Démontrer le produit eulérien : pour $s > 1$ réel,
...
\textbf{NT-25.} \begin{enumerate}
\item[(i)] Démontrer le théorème d'approximation de Liouville : si
...
$p/q$.
\item[(ii)] On dit que $\alpha$ est un nombre de Liouville si pour tout $n$ il existe un rationnel
...
\item[(iii)] Montrer que l'ensemble des nombres de Liouville est
...
\textbf{NT-26.} Démontrer le développement en fraction continue d'Euler
...
\textbf{NT-27.} Soit $N(r)$ le nombre de points $(a, b)$ du
réseau des entiers vérifiant $a^2 + b^2 \le r^2$.
\begin{enumerate}
\item[(i)] Démontrer l'estimation de Gauss :
...
\item[(iii)] Améliorer
l'erreur jusqu'à $O(r^{2/3})$ (l'exposant de Sierpi\'nski, atteignable par des estimations
élémentaires sur des sommes de parties fractionnaires).
\item[(iv)] Énoncer ce qui est connu et ce qui est conjecturé : on s'attend à ce que la vérité soit
...
bord. Tout ce qui suit est une lutte contre le bord : l'exposant 2/3 de Sierpi\'nski, en 1906, vint de
...
\textbf{NT-28.} (i) Trouver toutes les solutions entières de
...
\textbf{NT-29.} Un entier strictement positif $n$ est \textit{congruent} s'il
est l'aire d'un triangle rectangle dont les trois côtés sont rationnels.
\begin{enumerate}
\item[(i)] Montrer que $6$ est
congruent, et que $5$ l'est aussi (il existe un triangle de côtés de l'angle droit $3/2$ et
$20/3$).
\item[(ii)] Démontrer que $1$ n'est pas congruent --- de façon équivalente, mener la descente
infinie de Fermat montrant que $x^4 - y^4 = z^2$ n'a pas de solution en entiers strictement
positifs.
\item[(iii)] Démontrer le dictionnaire : les triangles rectangles rationnels d'aire $n$
correspondent aux points rationnels $(x, y)$ avec $y \ne 0$ sur la courbe elliptique
$y^2 = x^3 - n^2 x$.
\item[(iv)] Énoncer le théorème de Tunnell (1983), qui décide la congruence en
...
\textbf{NT-40.} À l'aide de la réciprocité quadratique et de ses lois complémentaires, évaluer le symbole de Legendre $ \left( \frac{1001}{9973} \right) $, en montrant chaque étape de réduction ; $ 9973 $ est premier.
\end{problem}
...
\textbf{NT-41.} Énoncer le lemme de Hensel, et s'en servir pour démontrer que $ x^2 \equiv 2 $ admet une solution dans les entiers $ 7 $-adiques $ \mathbb{Z}_7 $ mais que $ x^2 \equiv 2 $ n'en admet aucune dans $ \mathbb{Z}_5 $.
\end{problem}
...
\textbf{NT-49.} Pour un entier impair strictement positif $ n = \prod_i p_i^{e_i} $ et un
...
\textbf{NT-50.} Théorème de Dirichlet : si $ \gcd(a, m) = 1 $ alors la progression
...
\textbf{NT-51.} Énoncer le théorème de Zsigmondy pour $ a^n - b^n $ en
...
\textbf{NT-30.} Conjecture (forte) de Goldbach : tout entier pair
...
\begin{enumerate}
\item[(i)] démontrer que la conjecture forte
implique la faible ;
\item[(ii)] apprendre la méthode du cercle assez bien pour expliquer pourquoi les sommes de
\textit{trois} nombres premiers sont abordables alors que celles de deux ne le sont pas ;
\item[(iii)] lire l'énoncé
...
\textbf{NT-31.} La conjecture des nombres premiers jumeaux --- une infinité de nombres premiers
...
\item[(iii)] lire la percée de Zhang de 2013 et le crible de Maynard--Tao qui l'a suivie, et
expliquer ce que chacun apporte ;
\item[(iv)] expliquer le problème de parité de la théorie des cribles --- la
...
\textbf{NT-32.} Y a-t-il toujours un nombre premier entre $n^2$ et
...
\textbf{NT-33.} Un nombre est parfait s'il est égal à la somme de ses
...
\begin{enumerate}
\item[(i)] démontrer le théorème d'Euclide-Euler : les
...
\begin{problem}[{La conjecture d'Erd\H{o}s--Straus --- Recherche}]
\textbf{NT-34.} Erd\H{o}s et Straus ont conjecturé (1948) : pour tout
...
\begin{enumerate}
\item[(i)] démontrer la conjecture pour tout $n$ pair ;
\item[(ii)] la démontrer pour tout $n \equiv 3 \pmod 4$ ;
\item[(iii)] démontrer qu'un contre-exemple minimal
doit être premier ;
\item[(iv)] étudier l'approche par congruences couvrantes (Mordell), qui règle
...
Erd\H{o}s, fidèle à lui-même, transforma une notation antique en un problème ouvert précis. Le calcul
...
\begin{refs}
\item Contexte : Conjecture d'Erd\H{o}s-Straus --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Conjecture_d%27Erd%C5%91s-Straus})
\item Contexte : Fraction égyptienne --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Fraction_%C3%A9gyptienne})
\item Article : S. E. Salez, \og{} The Erd\H{o}s--Straus conjecture: new modular equations and checking up to N = 10\textasciicircum{}17 \fg{} (2014), arXiv:1406.6307 (\url{https://arxiv.org/abs/1406.6307})
...
\textbf{NT-35.} Pour un entier strictement positif $n$, soit
...
$b = 3^{2^k} - 1$ ;
\item[(ii)] en admettant abc, démontrer le \og{} Fermat asymptotique \fg{} :
$x^n + y^n = z^n$ n'a pas de solution en entiers premiers entre eux dès que $n$ est grand ;
\item[(iii)] faire le tour de ce qu'elle implique par ailleurs --- Mordell effectif (Elkies), la conjecture de
Szpiro, et davantage.
...
\textbf{NT-43.} Vérifier à la main la conjecture de Goldbach pour tout nombre pair de $ 4 $ à $ 100 $, en notant pour chacun une représentation en somme de deux nombres premiers. Expliquer ensuite, en un paragraphe, pourquoi étendre votre table jusqu'à $ 4 \times 10^{18} $ --- comme cela a effectivement été fait --- ne constitue absolument aucun progrès vers une démonstration.
\end{problem}
...
\textbf{NT-52.} Le théorème de Thue--Siegel--Roth : si $ \alpha $ est un nombre algébrique
...
\textbf{NT-53.} (\textbf{Ouvert.}) S. S. Pillai conjectura en 1931 que tout
...
Mih\u{a}ilescu (NT-45), chaque couple d'exposants fixé est réglé effectivement par la méthode de Baker,
