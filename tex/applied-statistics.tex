...
\textbf{STA-04.} Étant donné des points $ (x_1, y_1), \dots, (x_n, y_n) $ dont les $ x_i $
...
équations normales que vous établissez ici sont résolues des millions de fois par seconde au c\oe{}ur
...
\textbf{STA-05.} Des observations indépendantes $ x_1, \dots, x_n $ sont tirées de chacun des
...
\textbf{STA-06.} Soient $ x_1, \dots, x_n $ des tirages indépendants d'une loi quelconque
...
\textbf{STA-07.} Soient $ x_1, \dots, x_n $ des observations indépendantes
...
\textbf{STA-08.} Soit $ T $ une statistique de test de loi continue sous l'hypothèse nulle
...
\textbf{STA-09.} On teste une hypothèse nulle simple $ H_0 $ : les données ont pour densité
...
\textbf{STA-11.} Les numéros de série $ 1, 2, \dots, N $ sont estampillés sur $ N $ chars ;
...
\textbf{STA-22.} Soient $ x_1, \dots, x_n $ des tirages indépendants dans la
...
\textbf{STA-23.} Vous testez $ m $ hypothèses nulles et rejetez $ H_i $
...
\textbf{STA-24.} Prenez le modèle de régression simple normal : les $ x_i $ sont fixés et non
tous égaux, et
...
loi normale multivariée --- cette indépendance est un miracle purement gaussien, et c'est le c\oe{}ur du
problème.)
...
\textbf{STA-33.} Soient $ X_1, \dots, X_n $ indépendantes de fonction de
...
\textbf{STA-34.} Un \textit{collider} est une variable influencée par deux autres. Conditionner
...
\textbf{STA-12.} Soient $ x_1, \dots, x_n $ des tirages indépendants d'une loi $ F $, et
...
\textbf{STA-13.} Une statistique $ T $ est exhaustive pour un paramètre $ \theta $ si la loi
...
\textbf{STA-14.} Pour un modèle statistique régulier de densité $ f(x; \theta) $, le score est
...
$ \operatorname{Var}(W) \ge \frac{1}{n\, I(\theta)} $. (Le c\oe{}ur de la démonstration est
l'inégalité de Cauchy-Schwarz appliquée à la covariance de $ W $ avec le score.)
\item[(c)] Calculez $ I(\theta) $ pour les modèles Bernoulli($ p $), Poisson($ \lambda $) et
$ N(\mu, \sigma^2) $ à $ \sigma $ connu, et exhibez dans chaque cas un estimateur sans biais
atteignant la borne.
\item[(d)] Expliquez (démonstration non exigée) en quel sens l'EMV est asymptotiquement normal de variance
...
\textbf{STA-15.} On observe un unique $ X = (X_1, \dots, X_p) $ à
...
\textbf{STA-25.} Supposons que $ \sqrt{n}\,(T_n - \theta) $ converge en loi
...
\textbf{STA-26.} Soit $ Y = f(x) + \varepsilon $ en un point fixé $ x $,
...
\textbf{STA-27.} Un détecteur attribue à chaque objet un score réel $ s(X) $. Soit $ X_1 $
...
\textbf{STA-35.} Soit $ X \sim \mathrm{Binomial}(n, p) $ et estimons
...
\textbf{STA-36.} Supposons que les données observées $ y $ soient une fonction de données
...
\textbf{STA-37.} Chaque sujet $ i $ possède un temps de survie $ T_i $ et un temps de censure
...
\textbf{STA-38.} Un modèle à $ k $ paramètres libres est ajusté à $ n $ observations
...
\textbf{STA-16.} La pratique moderne choisit la question après avoir vu les données --- le plus
...
\textbf{STA-17.} Vers 2011--2015, il est apparu qu'une large part des résultats publiés en
...
\textbf{STA-18.} Une \textit{variable e} pour une hypothèse nulle $ H_0 $ est une variable
...
pratique scientifique ordinaire --- jeter un \oe{}il aux données, les mettre en commun, poursuivre ---
...
\textbf{STA-28.} La prédiction conforme par découpage ajuste un prédicteur
...
\textbf{STA-29.} Dans le modèle de covariance à pic, on observe $ n $
...
\textbf{STA-30.} Dans le modèle des résultats potentiels, chaque unité $ i $ porte deux
...
\textbf{STA-39.} En régression linéaire avec plus de prédicteurs que
...
\textbf{STA-40.} Un algorithme randomisé $ M $ est $ \varepsilon $-\textit{différentiellement
