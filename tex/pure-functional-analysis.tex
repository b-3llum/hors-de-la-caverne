...
\textbf{FA-01.} Soit $ C[0,1] $ l'espace vectoriel des fonctions continues à valeurs réelles
...
\textbf{FA-02.} Un \textit{espace de Banach} est un espace vectoriel normé complet pour la distance
...
\textbf{FA-03.} Soit $ H $ un espace de Hilbert --- un espace vectoriel muni d'un produit
...
\textbf{FA-04.} Soit $ H $ un espace de Hilbert et $ (e_n) $ une suite orthonormée de
$ H $.
\begin{enumerate}
...
\textbf{FA-05.} On travaille dans l'espace de Hilbert $ L^2(-\pi, \pi) $ muni du produit
...
\textbf{FA-06.} Soient $ X, Y $ des espaces normés et $ T : X \to Y $ linéaire. On définit la
norme d'opérateur
\[ \|T\| = \sup\{\, \|Tx\| : \|x\| \le 1 \,\}, \]
...
\textbf{FA-07.} Le principe de contraction, et la raison pour laquelle les problèmes de Cauchy ont
...
\textbf{FA-08.} Le dual $ X^{*} $ d'un espace normé $ X $ est l'espace de Banach des formes
...
\textbf{FA-19.} Deux normes $ \|\cdot\| $ et $ \|\cdot\|' $ sur un espace vectoriel $ X $
...
\textbf{FA-20.} Soit $ X $ un espace de Banach et $ T $ un opérateur borné sur $ X $ avec
$ \|T\| < 1 $.
\begin{enumerate}
...
\textbf{FA-21.} Soit $ f $ une forme linéaire non nulle sur un espace normé
...
\textbf{FA-22.} Démontrez que dans un espace préhilbertien réel le produit
...
\textbf{FA-30.} Pour un espace normé $ X $, on définit l'\textit{application canonique}
...
\textbf{FA-31.} On dit qu'une suite $ (e_n) $ d'un espace de Banach $ X $ est
...
\begin{problem}[{Hahn--Banach à l'\oe{}uvre --- Master}]
\textbf{FA-09.} Démontrez le théorème de prolongement, puis mettez-le au travail.
\begin{enumerate}
...
\textbf{FA-10.} La catégorie de Baire verse son premier dividende : une fonction continue dont la
...
\textbf{FA-11.} Deux des grands théorèmes de Banach, et un cas de continuité offerte.
\begin{enumerate}
...
\textbf{FA-12.} Un opérateur $ T $ sur un espace de Hilbert $ H $ est
...
\textbf{FA-13.} Soit $ T $ un opérateur compact autoadjoint sur un espace de Hilbert
$ H $.
\begin{enumerate}
...
\textbf{FA-14.} Une suite $ (x_n) $ d'un espace de Hilbert $ H $
...
\textbf{FA-15.} La relation de commutation de Heisenberg ne peut pas vivre dans $ B(H) $ ; ce
...
\textbf{FA-16.} Une \textit{C*-algèbre} est une algèbre de Banach $ A $ munie d'une involution
...
canonique $ X $, et toute C*-algèbre se plonge dans un $ B(H) $. La question (d) est le c\oe{}ur
...
\textbf{FA-23.} Soit $ H $ un espace de Hilbert complexe et $ T \in B(H) $. Le théorème de
...
\textbf{FA-24.} Le \textit{spectre} $ \sigma(T) $ de $ T \in B(H) $ est l'ensemble des
...
opérateurs et analyse complexe et raison pour laquelle les opérateurs de type décalage sont au c\oe{}ur des
...
\textbf{FA-25.} Soit $ H $ un espace de Hilbert et $ K $ un opérateur compact sur $ H $
(FA-12).
\begin{enumerate}
...
\textbf{FA-26.} Soit $ P $ un opérateur borné sur un espace de Hilbert avec
...
\textbf{FA-27.} Soit $ X $ un espace normé séparable. Démontrez que toute suite
...
\textbf{FA-32.} Soit $ A $ une algèbre de Banach complexe unifère et
$ a \in A $, et posons
...
\textbf{FA-33.} Soit $ A $ une algèbre de Banach complexe commutative unifère. Un
...
\textbf{FA-34.} Soit $ \mathcal{S}(\mathbb{R}) $ l'espace des fonctions lisses dont toutes les
...
\textbf{FA-35.} On dit que $ u \in L^2(0,1) $ admet la \textit{dérivée faible}
...
\textbf{FA-36.} Soit $ H $ un espace de Hilbert réel et soit
...
\textbf{FA-17.} (Ouvert pour l'espace de Hilbert.) Soit $ H $ un espace de Hilbert complexe
...
\textbf{FA-18.} (Résolu par la négative, 2020.) Un facteur
...
\textbf{FA-28.} (Ouvert.) Tout espace de Banach $ X $ de dimension infinie
...
\textbf{FA-29.} (Ouvert.) L'\textit{image numérique} d'un opérateur borné $ A $ sur
...
\textbf{FA-37.} (Résolu par l'affirmative, 2013.) Un \textit{état} sur une C*-algèbre est une forme
...
\textbf{FA-38.} (Ouvert.) Pour un groupe discret $ G $, soit $ \lambda $ la représentation
...
libres de Voiculescu, les facteurs de groupes libres interpolés $ L(F_t) $ de Dykema et R\u{a}dulescu, et
...
Dykema et R\u{a}dulescu ont montré indépendamment au début des années 1990 que les facteurs interpolés
...
\textbf{FA-39.} (Ouvert.) L'inégalité de Grothendieck affirme l'existence d'une
...
son public qu'il fut ignoré quinze ans, jusqu'à ce que Lindenstrauss et Pe\l{}czy\'nski le réécrivent dans le
