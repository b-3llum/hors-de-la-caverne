...
\textbf{TOP-01.} \textbf{Problème.} La ville de Königsberg comptait sept ponts reliant deux îles fluviales aux deux
...
\textbf{TOP-02.} \textbf{Problème.} Soit un polyèdre convexe à $ V $ sommets, $ E $ arêtes et $ F $
faces.
\begin{enumerate}
...
\textbf{TOP-03.} \textbf{Problème.} Fabriquez un ruban de Möbius : donnez un demi-tour à une bande de papier et collez
...
\textbf{TOP-04.} \textbf{Problème.} Supposez que la température varie continûment le long de
...
\textbf{TOP-25.} \textbf{Problème.} Soit $ f : [0,1] \to [0,1] $ continue. Démontrez qu'il existe
...
\textbf{TOP-26.} \textbf{Problème.} Soit $ P $ un polygone simple fermé du plan (un nombre fini de
...
\textbf{TOP-27.} \textbf{Problème.} Trois maisons doivent chacune être reliées à trois services --- eau, gaz,
...
qu'il n'existe pas est un petit chef-d'œuvre de la méthode topologique : rien sur les conduites, un
...
\textbf{TOP-37.} \textbf{Problème.} Deux profils de montagne sont donnés par des fonctions continues
...
\textbf{TOP-38.} \textbf{Problème.} Construisez un cadre de tableau polyédrique --- un bloc
...
\textbf{TOP-05.} \textbf{Problème.} Une distance sur un ensemble $ X $ est une fonction
...
\textbf{TOP-06.} \textbf{Problème.} \og{} Ouvert \fg{} et \og{} fermé \fg{} ne sont pas des contraires ; ce problème calibre les deux
mots.
\begin{enumerate}
...
\textbf{TOP-07.} \textbf{Problème.} Un espace est \textit{compact} si tout recouvrement par des ouverts admet un
...
\textbf{TOP-08.} \textbf{Problème.} Un espace est \textit{connexe} s'il n'est pas la réunion de deux ouverts disjoints
...
\textbf{TOP-09.} \textbf{Problème.} Le quotient d'un espace par une identification porte la topologie la plus fine
...
\textbf{TOP-10.} \textbf{Problème.} Un \textit{invariant topologique} est une propriété préservée par tout
homéomorphisme.
\begin{enumerate}
...
\textbf{TOP-11.} \textbf{Problème.} Subdivisez un triangle $ ABC $ en petits triangles, et coloriez chaque sommet de
...
\textbf{TOP-12.} \textbf{Problème.} Pour un espace $ X $ muni d'un point base $ x_0 $, soit
...
\item Lecture : A. Hatcher, \textit{Algebraic Topology}, ch. 1 --- gratuit sur la page de l'auteur (en anglais) (\url{https://pi.math.cornell.edu/\textasciitilde{}hatcher/AT/ATpage.html})
...
\textbf{TOP-28.} \textbf{Problème.} Une surjection $ q : X \to Y $ est une \textit{application
...
\textbf{TOP-29.} \textbf{Problème.} Soit $ (X,d) $ un espace métrique compact et soit
...
\textbf{TOP-30.} \textbf{Problème.} Sur un produit infini $ \prod_{\alpha \in A} X_\alpha $ il existe deux topologies
...
\textbf{TOP-39.} \textbf{Problème.} Une \textit{courbe remplissante} est une surjection continue
...
\textbf{TOP-40.} \textbf{Problème.} Démontrez que le ruban de Möbius se rétracte par déformation sur
...
\item Contexte : Ruban de Möbius --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Ruban_de_M%C3%B6bius})
\item Lecture : A. Hatcher, \textit{Algebraic Topology}, ch. 0 --- gratuit sur la page de l'auteur (en anglais) (\url{https://pi.math.cornell.edu/\textasciitilde{}hatcher/AT/ATpage.html})
\end{refs}

\bigskip

\begin{problem}[{Construire des espaces avec des cellules --- Licence}]
\textbf{TOP-41.} \textbf{Problème.} Un \textit{CW-complexe} fini se construit à partir d'un ensemble fini de points en
...
\item Lecture : A. Hatcher, \textit{Algebraic Topology}, ch. 0 et appendice --- gratuit sur la page de l'auteur (en anglais) (\url{https://pi.math.cornell.edu/\textasciitilde{}hatcher/AT/ATpage.html})
...
\textbf{TOP-13.} \textbf{Problème.} TOP-07 a montré que compacité par recouvrements et compacité séquentielle
...
\textbf{TOP-14.} \textbf{Problème.} On admet le calcul $ \pi_1(S^1) \cong \mathbb{Z} $ de TOP-12.
\begin{enumerate}
...
\textbf{TOP-15.} \textbf{Problème.} Un champ de vecteurs tangent sur la sphère $ S^2 $ associe continûment à chaque
...
\item Lecture : A. Hatcher, \textit{Algebraic Topology}, ch. 2 --- gratuit sur la page de l'auteur (en anglais) (\url{https://pi.math.cornell.edu/\textasciitilde{}hatcher/AT/ATpage.html})
...
\textbf{TOP-16.} \textbf{Problème.} Ce que toute application de la sphère vers le plan doit faire subir à un couple de
points antipodaux.
\begin{enumerate}
...
\noindent Stanisław Ulam a posé la conjecture et Karol Borsuk l'a démontrée en 1933 ; le
...
\textit{Using the Borsuk--Ulam Theorem} de Matoušek.
...
\textbf{TOP-17.} \textbf{Problème.} Un revêtement $ p : \tilde{X} \to X $ est une surjection continue dont chaque
...
\textbf{TOP-18.} \textbf{Problème.} Le théorème de Seifert--Van Kampen calcule $ \pi_1(U \cup V) $ à partir de
...
\textbf{TOP-19.} \textbf{Problème.} Démontrez le théorème de classification : toute surface compacte connexe (variété
...
\textbf{TOP-20.} \textbf{Problème.} Pour un complexe simplicial, le $ n $-ième groupe de chaînes est le groupe
...
\begin{problem}[{Le nœud de trèfle est vraiment noué --- Master}]
\textbf{TOP-21.} \textbf{Problème.} Un nœud est un cercle plongé dans $ \mathbb{R}^3 $, étudié à travers ses
diagrammes ; deux diagrammes représentent le même nœud exactement lorsqu'ils sont reliés par des
...
c'est un invariant de nœud.
\item[(b)] Montrez que le diagramme standard du nœud de trèfle est tricoloriable et que le diagramme rond du
nœud trivial ne l'est pas, et concluez que le trèfle ne peut pas être dénoué.
\item[(c)] Montrez que le nœud de huit n'est \textit{pas} tricoloriable, et expliquez ce que cela vous apprend
...
l'invariance, et trouvez un $ p $ qui distingue le nœud de huit du nœud trivial.
\end{enumerate}

\end{problem}

\noindent La théorie des nœuds a commencé avec la spéculation de Kelvin en 1867 selon laquelle
les atomes seraient des tubes de tourbillon noués, ce qui a lancé Tait dans la tabulation des nœuds ; les
mathématiques ont survécu à la physique. Reidemeister a démontré en 1927 que ses mouvements suffisent,
rendant l'équivalence de nœuds combinatoire, et l'astuce du tricoloriage --- avatar des représentations du
groupe du nœud, introduit par Ralph Fox vers 1956 --- est la démonstration honnête la plus simple qu'un nœud
est non trivial. La question (d) ouvre la porte à l'industrie moderne des invariants de nœuds : Alexander,
...
\item Contexte : Nœud de trèfle --- Wikipédia (\url{https://fr.wikipedia.org/wiki/N%C5%93ud_de_tr%C3%A8fle}), Mouvements de Reidemeister --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Mouvements_de_Reidemeister})
...
\textbf{TOP-31.} \textbf{Problème.} Pour un espace localement compact séparé $ X $, soit
...
\textbf{TOP-32.} \textbf{Problème.} Soit $ f : S^n \to S^n $ continue et non surjective. Démontrez
...
\item Lecture : A. Hatcher, \textit{Algebraic Topology}, \S{}2.2 --- gratuit sur la page de l'auteur (en anglais) (\url{https://pi.math.cornell.edu/\textasciitilde{}hatcher/AT/ATpage.html})
\end{refs}

\bigskip

\begin{problem}[{Le lemme d'Urysohn, et ce qu'achète la séparation --- Master}]
\textbf{TOP-33.} \textbf{Problème.} On dit qu'un espace topologique $ X $ est \textit{normal} si les singletons sont
...
\textbf{TOP-42.} \textbf{Problème.} Soit $ h : S^3 \to S^2 $ l'application de Hopf, définie sur les
...
\item Lecture : A. Hatcher, \textit{Algebraic Topology}, \S{}4.2 --- gratuit sur la page de l'auteur (en anglais) (\url{https://pi.math.cornell.edu/\textasciitilde{}hatcher/AT/ATpage.html})
\end{refs}

\bigskip

\begin{problem}[{Couper les espaces en deux : Mayer--Vietoris --- Master}]
\textbf{TOP-43.} \textbf{Problème.} Soit $ X = A \cup B $ avec $ A, B $ ouverts (ou une paire CW avec les
...
apparaît la torsion.
\item[(d)] Soit $ K \subseteq S^3 $ un nœud de voisinage tubulaire $ N $. Appliquez la suite à
$ A = N $ et $ B = S^3 \setminus K $ --- leur intersection est un tore --- et démontrez que
$ H_1(S^3 \setminus K) \cong \mathbb{Z} $ pour \textit{tout} nœud. Concluez que l'homologie seule ne peut
jamais distinguer le trèfle du nœud trivial, et dites ce que TOP-21 utilise à la place.
\end{enumerate}
...
de la dualité d'Alexander, et sa conclusion négative est la raison pour laquelle la théorie des nœuds a dû
...
\textbf{TOP-44.} \textbf{Problème.} Pour un CW-complexe fini $ X $ (TOP-41) et une application continue
...
\item Contexte : Degré d'une application continue --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Degr%C3%A9_d'une_application})
\item Lecture : A. Hatcher, \textit{Algebraic Topology}, \S{}2.C --- gratuit sur la page de l'auteur (en anglais) (\url{https://pi.math.cornell.edu/\textasciitilde{}hatcher/AT/ATpage.html})
\item Cours : MIT OCW 18.905 Algebraic Topology I (en anglais) (\url{https://ocw.mit.edu/courses/18-905-algebraic-topology-i-fall-2016/})
\end{refs}
...
\textbf{TOP-22.} \textbf{Problème.} (Ouvert.) La question de Poincaré lisse dans la seule dimension où elle tient
encore.
\begin{enumerate}
...
théorie des nœuds (Freedman--Gompf--Morrison--Walker 2010 ; Manolescu--Piccirillo 2021) restent actives ; des
...
\textbf{TOP-23.} \textbf{Problème.} (Ouvert.) Un nœud de $ S^3 = \partial B^4 $ est \textit{bordant} (slice) s'il borde
...
notions coïncident.
\begin{enumerate}
\item[(a)] Démontrez que tout nœud ruban est bordant.
\item[(b)] Montrez que la somme connexe d'un nœud quelconque avec son image miroir inversée est ruban (les
nœuds bordants sont donc abondants).
\item[(c)] Étudiez pourquoi le caractère bordant est véritablement quadridimensionnel.
\item[(d)] La conjecture elle-même : démontrez que tout nœud bordant est ruban, ou trouvez un
contre-exemple.
\end{enumerate}
...
central de la théorie des nœuds en dimension quatre, situé juste sous TOP-22 --- le caractère bordant de
nœuds particuliers est intimement lié à la recherche de sphères de dimension quatre exotiques. La
conjecture est démontrée pour de grandes familles : nœuds à deux ponts (Lisca 2007, via une théorie de
jauge à la Donaldson), nœuds pretzel à trois brins (Greene--Jabuka 2011), et d'autres ; des
...
\begin{refs}
\item Contexte : Nœud bordant --- Wikipédia (\url{https://fr.wikipedia.org/wiki/N%C5%93ud_bordant}), Nœud ruban --- Wikipédia (en anglais) (\url{https://en.wikipedia.org/wiki/Ribbon_knot})
...
\begin{problem}[{Reconnaître le nœud trivial : à quel prix ? --- Recherche}]
\textbf{TOP-24.} \textbf{Problème.} (Partiellement résolu ; la question centrale est ouverte.) RECONNAISSANCE DU NŒUD
TRIVIAL est le problème de décision suivant : étant donné un diagramme de nœud à $ n $ croisements,
est-ce un diagramme du nœud trivial ?
\begin{enumerate}
...
annoncé un algorithme de reconnaissance du nœud trivial s'exécutant en temps quasi polynomial
$ n^{O(\log n)} $. En 2026, aucun algorithme en temps polynomial n'est connu et aucun n'est démontré
impossible ; savoir si RECONNAISSANCE DU NŒUD TRIVIAL est dans P est un problème authentiquement ouvert, à
cheval sur la topologie et l'informatique.

\begin{refs}
\item Contexte : Problème de reconnaissance du nœud trivial --- Wikipédia (en anglais) (\url{https://en.wikipedia.org/wiki/Unknotting_problem})
...
\textbf{TOP-34.} \textbf{Problème.} (Ouvert.) On dit qu'un CW-complexe connexe est \textit{asphérique} si
...
\textbf{TOP-35.} \textbf{Problème.} (Ouvert.) Un polyèdre fini \textit{s'effondre} s'il peut être réduit
...
\textbf{TOP-36.} \textbf{Problème.} (Ouvert.) Soit $ M $ une variété lisse close orientée de groupe fondamental
...
\textbf{TOP-45.} \textbf{Problème.} (Résolu très récemment --- voir la note.) Une variété lisse close
...
\textbf{TOP-46.} \textbf{Problème.} (Ouvert.) Une variété close est \textit{asphérique} si son revêtement universel est
