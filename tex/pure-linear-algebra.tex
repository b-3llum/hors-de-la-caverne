...
\textbf{LIN-04.} Des vecteurs $v_1, \dots, v_k$ d'un espace vectoriel sont
...
\item[(ii)] En conclure que deux bases quelconques d'un espace vectoriel
de type fini ont le même cardinal --- la \textit{dimension} est donc bien définie.
\item[(iii)] Démontrer que toute famille libre
s'étend en une base, et que toute famille génératrice en contient une.
\item[(iv)] Montrer que les nombres réels forment
...
\textbf{LIN-05.} Soit $T : V \to W$ une application linéaire entre
espaces vectoriels de dimension finie.
\begin{enumerate}
\item[(i)] Démontrer le théorème du rang :
$\dim V = \dim \ker T + \dim \operatorname{im} T$.
\item[(ii)] En déduire qu'un système homogène de
...
\item[(iv)] Démontrer que le rang par lignes est égal au rang par colonnes, pour toute matrice.
\end{enumerate}
...
\textbf{LIN-06.} \begin{enumerate}
\item[(i)] Démontrer qu'il existe exactement une fonction $\det$ sur les
...
\item[(ii)] À partir de ces seuls axiomes, démontrer
...
tout pavé par $|\det T|$.
\item[(iv)] Expliquer, avec démonstration, pourquoi une matrice est inversible exactement lorsque son
...
\textbf{LIN-07.} Un vecteur non nul $v$ est \textit{vecteur propre} de la
matrice carrée $A$ pour la \textit{valeur propre} $\lambda$ si $Av = \lambda v$.
\begin{enumerate}
\item[(i)] Démontrer que
les valeurs propres de $A$ sont exactement les racines du polynôme caractéristique
$\det(A - \lambda I)$.
\item[(ii)] Démontrer que des vecteurs propres associés à des valeurs propres distinctes sont
linéairement indépendants.
\item[(iii)] Les nombres de Fibonacci vérifient
...
\textbf{LIN-08.} Une matrice carrée est \textit{diagonalisable} si elle est semblable
à une matrice diagonale, c'est-à-dire $A = PDP^{-1}$.
\begin{enumerate}
\item[(i)] Démontrer que $A$ (sur $\mathbb{C}$) est
...
\item[(iii)] Démontrer que toute famille commutative de matrices
diagonalisables est \textit{simultanément} diagonalisable.
\item[(iv)] Montrer que les matrices diagonalisables sont
...
\textbf{LIN-09.} Soit $p(\lambda) = \det(\lambda I - A)$ le
polynôme caractéristique d'une matrice $n \times n$ $A$.
\begin{enumerate}
\item[(i)] Le théorème de Cayley--Hamilton
...
aucun sens.
\item[(ii)] Donner une démonstration correcte, sur $\mathbb{C}$. (Une voie : la démontrer pour les matrices
diagonalisables et utiliser leur densité, LIN-08(iv) ; une autre : l'identité de la comatrice.)
\item[(iii)] En déduire que, pour $A$ inversible, l'inverse $A^{-1}$ est un polynôme en $A$ de degré au
plus $n - 1$.
\item[(iv)] Le théorème vaut-il sur un anneau commutatif quelconque ? Démontrer ou
réfuter.
\end{enumerate}
...
\textbf{LIN-10.} Soit $A$ une matrice réelle symétrique $n \times n$
($A = A^{\mathsf{T}}$). Démontrer :
\begin{enumerate}
\item[(i)] toute valeur propre de $A$ est réelle ;
\item[(ii)] des vecteurs propres associés à des
valeurs propres distinctes sont orthogonaux ;
\item[(iii)] $\mathbb{R}^n$ admet une base orthonormée de
vecteurs propres de $A$, c'est-à-dire $A = Q \Lambda Q^{\mathsf{T}}$ avec $Q$ orthogonale et
$\Lambda$ diagonale.
\item[(iv)] En déduire le théorème des axes principaux : toute forme quadratique
...
\textbf{LIN-11.} Une matrice réelle symétrique $A$ est \textit{définie positive}
...
\textbf{LIN-12.} Pour une matrice carrée $A$, on pose
$e^{A} = \sum_{k=0}^{\infty} A^k / k!$.
\begin{enumerate}
\item[(i)] Démontrer que la série converge pour toute $A$.
\item[(ii)] Démontrer que si $AB = BA$ alors $e^{A+B} = e^A e^B$, et trouver deux matrices pour lesquelles
$e^{A+B} \ne e^A e^B$.
\item[(iii)] Démontrer $\det e^A = e^{\operatorname{tr} A}$.
\item[(iv)] Montrer que
...
\textbf{LIN-13.} (i) Démontrer que toute matrice réelle $m \times n$ $A$ peut
...
\textbf{LIN-25.} Soit $A$ une matrice réelle $m \times n$ avec $m \ge n$
...
\textbf{LIN-26.} Soit $A$ une matrice réelle symétrique $n \times n$ de plus
...
\textbf{LIN-27.} Soient $A$ une matrice réelle $m \times n$ et $b \in \mathbb{R}^m$. Le
problème des moindres carrés consiste à minimiser $\|Ax - b\|$ sur tous les $x \in \mathbb{R}^n$,
où $\|\cdot\|$ est la norme euclidienne.
\begin{enumerate}
\item[(i)] Démontrer que $x$ minimise $\|Ax - b\|$ si et seulement si $Ax$ est la projection orthogonale
...
\item[(ii)] Démontrer que le minimiseur est unique exactement lorsque les colonnes de $A$ sont indépendantes,
...
\item[(iii)] Démontrer la réciproque de (ii) : toute matrice symétrique idempotente est la projection orthogonale
sur son propre espace des colonnes.
\item[(iv)] À l'aide de la factorisation $A = QR$ de LIN-25, montrer que les équations normales deviennent
...
aujourd'hui les mêmes équations normales siègent au c\oe{}ur de la régression, du positionnement GPS et
...
\textbf{LIN-36.} Exécuter le pivot de Gauss sur une matrice carrée $A$ sans échange de lignes,
...
\textbf{LIN-37.} Soit $A$ une matrice réelle $m \times n$. On dit qu'une matrice
...
\textbf{LIN-38.} Pour un polynôme unitaire
...
\textbf{LIN-14.} Soit $T$ un endomorphisme d'un espace vectoriel
complexe de dimension finie $V$.
\begin{enumerate}
\item[(i)] Démontrer que $V$ est la somme directe des \textit{sous-espaces
...
$N$.
\item[(iii)] Combiner (i) et (ii) pour
...
\item[(iv)] En conclure que deux matrices
...
\textbf{LIN-15.} \begin{enumerate}
\item[(i)] Démontrer le théorème de Perron : une matrice carrée à
...
\item[(ii)] Expliquer par un exemple ce qui peut échouer
...
\item[(iii)] PageRank modélise un internaute qui, depuis une page, suit un lien sortant au hasard avec
...
\textbf{LIN-16.} Soit $A$ une matrice réelle symétrique $n \times n$ de
...
\textbf{LIN-17.} \begin{enumerate}
\item[(i)] Montrer que des matrices $2 \times 2$ sur un anneau commutatif
...
\item[(ii)] Démontrer qu'appliquer cela récursivement à des matrices
par blocs multiplie deux matrices $n \times n$ en $O(n^{\log_2 7}) = O(n^{2{,}81})$ opérations
élémentaires.
\item[(iii)] Démontrer que 7 est optimal pour le cas $2 \times 2$ : aucun algorithme bilinéaire n'emploie 6
multiplications (Winograd, 1971 --- difficile ; a minima, lire une démonstration et en reconstruire le
plan).
\item[(iv)] Montrer que tout algorithme demande au moins $cn^2$ étapes, de sorte que l'exposant de la
...
\textbf{LIN-18.} \begin{enumerate}
\item[(i)] Définir le produit tensoriel $V \otimes W$ de deux espaces
...
\item[(ii)] Démontrer que tout élément de $V \otimes W$ n'est pas un tenseur \og{} pur \fg{} $v \otimes w$ ; le
...
\item[(iii)] Montrer que la multiplication des matrices $2 \times 2$
...
purs.
\item[(iv)] Contrairement au rang matriciel, le rang tensoriel à trois facteurs peut sauter \textit{vers le bas}
...
\textbf{LIN-28.} Soit $N$ une matrice $n \times n$ sur $\mathbb{C}$.
...
\textbf{LIN-29.} Soient $A$ et $B$ deux matrices complexes $n \times n$
...
\textbf{LIN-30.} Une \textit{circulante} est une matrice $C$ dont les lignes sont les décalages
...
\begin{enumerate}
\item[(i)] Démontrer que $S^n = I$ et que toute circulante est égale à $p(S)$ pour le polynôme
...
\item[(ii)] Démontrer que les vecteurs $v_k = (1, \omega^{k}, \omega^{2k}, \dots, \omega^{(n-1)k})^{\mathsf{T}}$,
...
\item[(iii)] En déduire que les circulantes forment une algèbre commutative isomorphe à
...
\item[(iv)] Démontrer que toute circulante est une matrice normale, et donner une formule pour $C^{-1}$
...
\textbf{LIN-39.} Soit $A$ une matrice complexe $n \times n$ et $A^{*}$ sa transconjuguée. On
...
\textbf{LIN-40.} Pour $A$ de taille $m \times m$ et $B$ de taille $n \times n$, le
...
\textbf{LIN-41.} On dit qu'une matrice $n \times n$ est \textit{bistochastique}
...
matriciel --- le n\oe{}ud de toute démonstration consiste à extraire une permutation du motif des
...
\textbf{LIN-19.} L'exposant $\omega$ est la borne inférieure de tous les $c$
...
\textbf{LIN-20.} Une \textit{matrice de Hadamard} d'ordre $n$ est une
...
\begin{enumerate}
\item[(i)] démontrer qu'une matrice de Hadamard d'ordre $n > 2$ force $n \equiv 0 \pmod 4$ ;
\item[(ii)] démontrer l'inégalité de Hadamard --- une matrice $n \times n$ à coefficients dans $[-1, 1]$
vérifie $|\det| \le n^{n/2}$ --- et montrer que l'égalité a lieu exactement pour les matrices de
Hadamard ;
\item[(iii)] construire des matrices de Hadamard de tous les ordres $2^k$ (Sylvester, 1867) et d'ordre 12
...
\textbf{LIN-21.} L'\textit{image numérique} d'une matrice carrée complexe
...
\begin{enumerate}
\item[(i)] démontrer que $W(A)$ est convexe (le
théorème de Toeplitz--Hausdorff) ;
\item[(ii)] démontrer la conjecture --- avec la constante 1 --- pour les matrices normales,
en utilisant le théorème spectral ;
\item[(iii)] montrer par un exemple que pour $A$ non normale, $\|p(A)\|$ peut
...
\textbf{LIN-31.} Une matrice de Hadamard circulante est une circulante (LIN-30) à
...
\textbf{LIN-32.} Le permanent d'une matrice $n \times n$ est
...
\textbf{LIN-33.} Une famille de droites passant par l'origine de $\mathbb{R}^d$ est
...
\begin{enumerate}
\item[(i)] Démontrer la borne de Gerzon $N(d) \le \tbinom{d+1}{2}$, en montrant que les projections
...
\item[(ii)] Démontrer la borne relative : si les droites sont engendrées par des vecteurs unitaires $u_i$
vérifiant $|\langle u_i, u_j \rangle| = \alpha$ pour $i \ne j$, et si $d < 1/\alpha^2$, alors
\[ N_\alpha(d) \;\le\; \frac{d\,(1 - \alpha^2)}{1 - d\,\alpha^2}. \]
\item[(iii)] Construire 6 droites équiangulaires dans $\mathbb{R}^3$ (les six diagonales d'un icosaèdre
régulier) et 28 dans $\mathbb{R}^7$, et démontrer que 6 est optimal dans $\mathbb{R}^3$.
\item[(iv)] Étudier le théorème de 2021 de Jiang, Tidor, Yao, Zhang et Zhao, qui détermine $N_\alpha(d)$
...
\item Article : I. Balla \& M. Buci\'c, \og{} Equiangular lines via improved eigenvalue multiplicity \fg{} (2024), arXiv:2409.16219 (\url{https://arxiv.org/abs/2409.16219})
\end{refs}

\bigskip

\begin{problem}[{À quelle vitesse un produit de matrices peut-il croître ? --- Recherche}]
\textbf{LIN-42.} Pour un ensemble fini $\mathcal{M} = \{A_1, \dots, A_m\}$ de matrices réelles
...
\textbf{LIN-43.} Soient $A$ et $B$ des matrices hermitiennes
