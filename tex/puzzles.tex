...
\textbf{PUZ-101.} Trois boîtes fermées portent les étiquettes \og{} pommes \fg{}, \og{} oranges \fg{} et
...
\textbf{PUZ-102.} Un escargot part du fond d'un puits de $ 10 $ mètres :
...
\textbf{PUZ-103.} Un carré magique d'ordre $ 3 $ est un tableau de $ 3 \times 3 $ cases
...
\textbf{PUZ-104.} Une balance à deux plateaux ne fait que comparer : elle dit quel côté
...
\textbf{PUZ-105.} Une tablette de chocolat est un rectangle de
...
\textbf{PUZ-106.} Un homme laisse dix-sept chameaux à ses trois fils : la moitié à l'aîné,
...
\textbf{PUZ-107.} Un verre contient un décilitre d'eau, un autre un
...
\textbf{PUZ-108.} On veut connaître la masse d'un éléphant. On dispose d'une rivière, d'une
...
\textbf{PUZ-109.} Une corde tendue fait exactement le tour de l'équateur
...
philosophique : vous devez arbitrer entre une formule que vous savez par c\oe{}ur et une
...
\textbf{PUZ-110.} Vous disposez d'une corde de longueur $ L $ et vous voulez enclore le
...
côte d'Afrique autant de terre qu'une peau de b\oe{}uf pouvait en couvrir ; elle fit découper la
...
\textbf{PUZ-111.} Sur du papier quadrillé, prenez le carré $ 8 \times 8 $ dont les coins
...
lamelle si allongée que l'\oe{}il la prend pour un trait de crayon. Derrière l'illusion se cache
...
\textbf{PUZ-112.} Deux trains roulent l'un vers l'autre sur la même voie, chacun à
...
\textbf{PUZ-113.} Huit joueurs disputent un tournoi à élimination directe : quatre matchs au
...
match une comparaison. La question (c) est le c\oe{}ur du sujet : pour minorer le nombre de matchs,
...
\textbf{PUZ-114.} On travaille dans le calendrier grégorien, celui qui est en usage
...
\textbf{PUZ-115.} On écrit des nombres en chiffres romains avec des allumettes : le
...
\textbf{PUZ-116.} Vous montez une côte de deux kilomètres à la vitesse
...
\textbf{PUZ-09.} Cent prisonniers sont numérotés de $ 1 $ à $ 100 $. Dans une pièce
...
\textbf{PUZ-10.} Cent prisonniers se tiennent en file, chacun coiffé d'un chapeau rouge
...
\textbf{PUZ-11.} Sur une île vivent $ 100 $ personnes aux yeux bleus et $ 100 $ aux
...
\textbf{PUZ-12.} Cent prisonniers sont conduits, un à la fois, dans un
...
\textbf{PUZ-13.} Deux personnes partagent un gâteau par \og{} je coupe, tu
...
\begin{problem}[{Deux \oe{}ufs et cent étages --- Licence}]
\textbf{PUZ-24.} Un immeuble compte $ 100 $ étages, et il existe un étage seuil $ f $ tel qu'un \oe{}uf lâché depuis l'étage $ f $ ou au-dessus se casse, tandis qu'un \oe{}uf lâché depuis un étage inférieur ne se casse pas. Un \oe{}uf qui survit à une chute est intact et peut resservir ; un \oe{}uf cassé est perdu. Vous devez déterminer $ f $ exactement, et vous êtes jugé sur le nombre de lâchers dans le pire des cas.
\begin{enumerate}
\item[(a)] Avec un seul \oe{}uf, démontrez que $ 100 $ lâchers peuvent être nécessaires et qu'aucune stratégie ne fait mieux dans le pire des cas.
\item[(b)] Avec deux \oe{}ufs, trouvez une stratégie qui minimise le nombre de lâchers dans le pire des cas, et démontrez qu'aucune stratégie ne fait mieux.
\item[(c)] Avec $ k $ \oe{}ufs et un budget de $ t $ lâchers, déterminez, avec démonstration, le plus grand nombre d'étages que l'on peut résoudre. Déduisez (b) de votre formule, et écrivez la récurrence qui exprime le problème général comme un programme dynamique.
\end{enumerate}

\end{problem}

\noindent La recherche dichotomique est indisponible ici, et la raison mérite d'être énoncée précisément : un essai qui renvoie \og{} cassé \fg{} vous coûte une ressource, de sorte que les deux branches de l'arbre de décision ne sont pas symétriques, et que l'arbre optimal est donc déséquilibré d'une manière que contrôle le nombre d'\oe{}ufs restants. C'est le point d'entrée standard, en classe, vers la programmation dynamique et vers la distinction entre optimiser le pire des cas et optimiser le cas moyen, qui donnent ici des stratégies différentes ; Moshe Sniedovich a rédigé le casse-tête avec soin à des fins pédagogiques en 2003, après l'avoir utilisé devant des auditoires d'étudiants à Brunswick et à Hong Kong. Sa provenance relève du folklore --- il a circulé pendant des années comme question d'entretien avant que quiconque prenne la peine de le publier --- mais la réponse générale à $ k $ \oe{}ufs de la question (c) est assez nette pour valoir la peine d'être devinée sur de petits cas puis démontrée en règle.

\begin{refs}
\item Contexte : Programmation dynamique : le casse-tête des \oe{}ufs lâchés --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Programmation_dynamique})
...
\textbf{PUZ-26.} En 1902, Henry Dudeney demanda comment découper un triangle équilatéral en pièces qui se réassemblent, sans chevauchement ni trou, en un carré de même aire.
\begin{enumerate}
...
\textbf{PUZ-27.} Un carré peut être découpé en un nombre fini de carrés plus petits, de tailles deux à deux distinctes : la plus petite dissection de ce type, trouvée par A. J. W. Duijvestijn en 1978, utilise $ 21 $ carrés et a pour côté $ 112 $. Démontrez qu'aucun cube ne peut être découpé en un nombre fini de cubes plus petits de tailles deux à deux distinctes.
\end{problem}
...
\textbf{PUZ-35.} Trois dieux, appelés A, B et C, sont --- dans un ordre que vous ignorez ---
...
\textbf{PUZ-36.} Un samedi, un juge dit à un prisonnier : vous serez pendu à midi l'un
...
\textbf{PUZ-37.} Une pièce tombe sur pile avec probabilité $ p $, où
...
\textbf{PUZ-38.} Deux généraux campés sur des collines opposées doivent
...
\textbf{PUZ-49.} Un triangle équilatéral est inscrit dans un cercle de rayon $ 1 $. On
...
\textbf{PUZ-50.} Le jeu de Wythoff se joue avec deux tas de jetons. Un coup retire soit un
...
a rapporté que le jeu se pratiquait en Chine sous le nom de \textit{ji\v{a}n shíz\v{\i}}, \og{} ramasser des
...
\textbf{PUZ-51.} Lors d'une élection, le candidat A reçoit $ p $ voix
...
\textbf{PUZ-52.} Un jeu \textit{impartial} est un jeu à deux joueurs sans hasard ni
...
\textbf{PUZ-64.} Une lumière de longueur d'onde $ \lambda $ traverse un gaz dilué de
...
\textbf{PUZ-65.} Des feuilles de thé reposent au fond plat d'une tasse cylindrique de
...
\textbf{PUZ-66.} De l'eau s'écoule par un trou de rayon $ a $ percé
...
\textbf{PUZ-67.} Une échelle de longueur propre $ L $ se déplace selon sa propre
...
\textbf{PUZ-79.} Prenez $ n $ personnes dont les dates d'anniversaire sont
...
\textbf{PUZ-80.} Un préposé prend les chapeaux de $ n $ invités et les rend dans un
...
\textbf{PUZ-81.} Fixez des entiers strictement positifs $ a_1, \dots, a_k $ de plus
...
\textbf{PUZ-82.} Un \textit{appariement de Langford} d'ordre $ n $ est
...
\textbf{PUZ-92.} Le Hex se joue sur un losange de $ n \times n $ cases hexagonales. Deux
...
\textbf{PUZ-93.} Chacun de $ n $ joueurs reçoit un chapeau, rouge ou bleu,
...
\textbf{PUZ-94.} Un mathématicien porte une boîte d'allumettes dans
...
\textbf{PUZ-95.} Il y a $ n $ hôpitaux et $ n $ candidats. Chaque hôpital dispose
...
une défense de leur article comme \oe{}uvre mathématique malgré l'absence de nombres, de formules
...
\textbf{PUZ-96.} Une urne contient $ 90 $ boules. Exactement $ 30 $ sont rouges ; les
...
\textbf{PUZ-14.} Une infinité dénombrable de prisonniers, indexés par $ \mathbb{N} $,
...
\textbf{PUZ-15.} Des pions occupent tous les points du réseau situés
...
\textbf{PUZ-28.} Deux polytopes sont \textit{équidécomposables} si l'on peut découper l'un en un nombre fini de morceaux polytopaux qui se réassemblent en l'autre, en n'utilisant que des translations et des rotations.
\begin{enumerate}
...
\textbf{PUZ-39.} Sur $ n $ enfants qui ont joué dehors, exactement $ k \ge 1 $ ont le
...
\textbf{PUZ-40.} Deux entiers $ x $ et $ y $ vérifient $ 1 < x < y $ et
...
\textbf{PUZ-41.} Deux enveloppes sont posées devant vous ; l'une contient deux fois plus
...
\textbf{PUZ-42.} Au Mastermind, le codeur choisit une suite secrète de
...
\textbf{PUZ-53.} Une position de Hackenbush bleu-rouge est un graphe fini dont les arêtes
...
expliquer la construction, et elle reste l'une des très rares \oe{}uvres mathématiques publiées
...
\textbf{PUZ-54.} Vous pariez de façon répétée sur une proposition favorable. À chaque tour
...
\textbf{PUZ-55.} Un point est choisi uniformément sur la sphère unité ;
...
\textbf{PUZ-68.} Modélisez un chat par deux cylindres rigides identiques et axisymétriques
...
\textbf{PUZ-69.} Une tête d'arroseur en forme de S est immergée dans un grand bac et
...
\textbf{PUZ-70.} Une toupie tippe-top est une sphère tronquée de rayon
...
\item Article : N. Bou-Rabee, J. E. Marsden \& L. A. Romero, \og{} Tippe top inversion as a dissipation-induced instability \fg{}, \textit{SIAM Journal on Applied Dynamical Systems} 3 (2004) --- exemplaire de l'auteur (\url{https://www.cds.caltech.edu/~marsden/bib/2004/04-BoMaRo2004/BoMaRo2004_tippe.pdf})
\end{refs}

\bigskip

\begin{problem}[{Un carré et un nombre impair de triangles --- Master}]
\textbf{PUZ-83.} Une \textit{équidissection} d'un polygone en $ m $ morceaux est une
...
\textbf{PUZ-84.} Sept joueurs reçoivent chacun un chapeau rouge ou bleu,
...
\textbf{PUZ-97.} Un lion et un homme, chacun réduit à un point, se
...
\textbf{PUZ-98.} Soit $ A $ un ensemble fini d'options avec $ |A| \ge 3 $ et soit
...
\textbf{PUZ-16.} Un carré magique est un tableau $ 3 \times 3 $ dont les trois lignes,
...
\textbf{PUZ-43.} Généralisez le jeu de PUZ-42. Le secret est un mot de $ [k]^{n} $ ;
...
et reconstituez le théorème d'Erd\H{o}s et Rényi (1963) selon lequel $ \Theta(n/\log n) $ essais
...
mathématiques sont plus anciennes que le jeu : l'article d'Erd\H{o}s et Rényi de 1963, \og{} On two
...
\begin{refs}
\item Contexte : Mastermind --- Wikipédia (\url{https://fr.wikipedia.org/wiki/Mastermind})
\item Article : Va\v{s}ek Chvátal, \og{} Mastermind \fg{}, \textit{Combinatorica} 3 (1983), 325--329
...
\textbf{PUZ-44.} Carole choisit $ x \in \{1, \ldots, n\} $. Paul pose des questions de
...
forme hongroise des vingt questions ; Stanis\l{}aw Ulam l'a reposée, indépendamment, dans son
...
\textbf{PUZ-56.} Le Chomp se joue sur une grille $ m \times n $ de carrés. Un coup
...
\item Article : Doron Zeilberger, \og{} Three-rowed CHOMP \fg{}, \textit{Advances in Applied Mathematics} 26 (2001), exemplaire de l'auteur (\url{https://sites.math.rutgers.edu/~zeilberg/mamarim/mamarimPDF/chomp.pdf})
\end{refs}

\bigskip

\begin{problem}[{Le problème de Robbins : le problème du secrétaire que personne n'a résolu --- Recherche}]
\textbf{PUZ-57.} Vous observez des variables aléatoires indépendantes
...
\item Lecture : Thomas S. Ferguson, \textit{Optimal Stopping and Applications (\url{https://www.math.ucla.edu/~tom/Stopping/Contents.html})} --- notes de cours en libre accès (en anglais) ; voir aussi Optimisation et théorie des jeux (\url{applied-optimization.html}) pour la conjecture du secrétaire matroïdal
\end{refs}

\bigskip

\begin{problem}[{La Belle au bois dormant --- Recherche, short}]
\textbf{PUZ-58.} On expose à l'avance le protocole à la Belle au bois
...
\textbf{PUZ-71.} Fixez un protocole de refroidissement : un récipient de géométrie et de
...
\textbf{PUZ-72.} Une longue chaîne à billes repose enroulée dans un bécher
...
\textbf{PUZ-85.} Écrivez les entiers strictement positifs en spirale carrée --- $ 1 $ au
...
\noindent Stanis\l{}aw Ulam a dessiné la spirale en 1963 en griffonnant pendant ce qu'il
...
\textbf{PUZ-86.} Soit $ P $ un polyèdre convexe. Un \textit{dépliage par arêtes} de
...
\textbf{PUZ-99.} On joue sur les cases d'un plateau infini $ \mathbb{Z}^{2} $. L'Ange de
...
\textbf{PUZ-100.} Un Prédicteur, dont les prévisions des choix humains se sont révélées
