...
\textbf{CRY-05.} Messages, clés et textes chiffrés sont des chaînes
...
\textbf{CRY-06.} Deux messages ont été chiffrés avec le
...
\item[(c)] Mettez votre
méthode à l'\oe{}uvre. Les deux textes clairs ci-dessous sont des phrases anglaises en
...
\textbf{CRY-07.} La cryptographie à clé publique calcule sans cesse
...
\textbf{CRY-08.} Fixez un grand nombre premier $p$ et un générateur $g$
...
\textbf{CRY-09.} Soit $n = pq$ avec $p, q$ premiers distincts,
...
Diffie--Hellman eut appelé de ses v\oe{}ux une telle fonction \og{} à trappe \fg{} (Clifford Cocks
...
\textbf{CRY-10.} Une fonction de hachage rend des valeurs dans un ensemble de taille
...
\textbf{CRY-11.} Pour une variable aléatoire $X$ prenant la valeur $i$
...
\textbf{CRY-12.} Un code binaire attribue à chacun de $n$ symboles
...
\textbf{CRY-27.} Fixez un groupe cyclique $G$ d'ordre $q$ de générateur $g$.
...
\textbf{CRY-28.} Soit $n = pq$ un module RSA d'exposant privé $d$ comme
...
\textbf{CRY-29.} Un code binaire de longueur $n$ est une partie
...
\textbf{CRY-30.} En admettant que le groupe multiplicatif
...
\textbf{CRY-31.} Avec l'entropie de CRY-11, démontrez que pour toute
...
\textbf{CRY-37.} Un chiffre est une fonction ; une \textit{implémentation} est une fonction
...
\textbf{CRY-43.} Un LFSR binaire de longueur $L$ détient un état
...
\textbf{CRY-44.} Le chaînage de blocs chiffrés répare le défaut de CRY-42. Avec des blocs
...
\textbf{CRY-45.} Shannon a défini la \textit{distance d'unicité} d'un chiffre
...
\textbf{CRY-13.} Soit $G$ un groupe cyclique d'ordre connu
...
\textbf{CRY-14.} Soit $E$ la courbe $y^2 = x^3 + ax + b$
...
\textbf{CRY-15.} Ceci est un problème de compréhension avec un vrai
théorème à l'intérieur.
\begin{enumerate}
\item[(a)] Démontrez la réduction classique au c\oe{}ur de l'algorithme de Shor :
...
\textbf{CRY-16.} Fixez un module $q$ et un vecteur secret
...
question (d), qui descend de la découverte d'Ajtai en 1996, en est le c\oe{}ur conceptuel : la
...
\textbf{CRY-17.} Peggy affirme que deux graphes publics $G_0, G_1$
...
\textbf{CRY-18.} Pour partager un secret $s \in \mathbb{F}_p$
...
\textbf{CRY-19.} Soit $X$ un symbole aléatoire d'entropie
...
\textbf{CRY-20.} Le canal binaire symétrique $\mathrm{BSC}(p)$
...
aux codes LDPC et aux codes polaires d'Ar\i{}kan (2009), la première famille explicite dont on
...
\textbf{CRY-32.} Soit $G$ un groupe cyclique d'ordre premier $n$ et de générateur
...
\textbf{CRY-33.} Le codage de Huffman (CRY-19) gaspille jusqu'à un bit \textit{par symbole},
...
\textbf{CRY-34.} Définissez l'entropie conditionnelle
...
\textbf{CRY-35.} Un schéma de mise en gage permet à un émetteur de publier
...
\textbf{CRY-38.} Fixez un schéma de chiffrement symétrique
...
\textbf{CRY-46.} Fixez $n$ et des fonctions $f_1, \dots, f_r \colon \{0,1\}^n \to
...
\textbf{CRY-47.} Soit $S \colon \{0,1\}^4 \to \{0,1\}^4$ une bijection (une boîte S) et
...
\textbf{CRY-48.} Dans un groupe cyclique $G = \langle g \rangle$ d'ordre
...
\textbf{CRY-21.} En août 2024, le NIST a finalisé ses premières
...
\textbf{CRY-22.} Un obfuscateur indistinguable (iO)
...
\textbf{CRY-23.} L'algorithme de Grover fouille un espace non
...
\textbf{CRY-39.} Démontrez que si des fonctions à sens unique existent
...
\textbf{CRY-40.} L'algorithme LLL (1982) produit, en temps polynomial,
...
\textbf{CRY-49.} La transformation de Fiat--Shamir rend non interactive une preuve
...
\textbf{CRY-50.} Dans le \textit{transfert inconscient} 1 parmi 2, un émetteur
