...
\textbf{PDE-01.} Soit $c > 0$ et soient $f, g : \mathbb{R} \to \mathbb{R}$
...
\textbf{PDE-02.} Une fonction $u$ à dérivées secondes continues est
...
\textbf{PDE-04.} Une barre mince et homogène a pour température $u(x,t)$,
...
\textbf{PDE-05.} On considère $u_t = k u_{xx}$ sur $0 < x < L$
...
\textbf{PDE-06.} (a) En factorisant l'opérateur des ondes en
...
\textbf{PDE-07.} Soit $f$ le signal carré : $f(x) = 1$ pour
...
\textbf{PDE-08.} Soit $u$ une fonction harmonique sur un ouvert
...
\textbf{PDE-09.} Jean Bernoulli, \textit{Acta Eruditorum},
...
\textbf{PDE-21.} Pour l'équation à coefficients constants
...
\textbf{PDE-22.} Pour l'équation des ondes $u_{tt} = c^2 u_{xx}$
...
\textbf{PDE-23.} Le principe de Duhamel affirme qu'une équation d'évolution linéaire
...
sous le signe intégral échoue en $s = t$, et réparer cette étape est le c\oe{}ur de la
démonstration.
...
\textbf{PDE-24.} Une membrane occupe le rectangle
...
\textbf{PDE-31.} Une membrane couvre le disque de rayon $a$, est encastrée le long de son
...
\textbf{PDE-32.} Écrire l'équation de Laplace en coordonnées sphériques et
...
\textbf{PDE-33.} Un \textit{problème de Sturm--Liouville régulier} sur $[a,b]$ est
...
\textbf{PDE-10.} (a) Démontrer le principe du maximum faible : une
...
\textbf{PDE-11.} (a) Montrer que si $u(x,t)$ résout
...
\textbf{PDE-12.} On considère une équation du premier ordre pleinement
...
\textbf{PDE-13.} On considère $u_t + u u_x = 0$ avec une donnée
...
\item[(d)] Calculer $T^*$ pour $\varphi(x) = e^{-x^2}$, et décrire géométriquement ce que
...
\textbf{PDE-14.} Écrire l'équation de Burgers sous forme conservative
...
\textbf{PDE-15.} On dit que $v$ est la \textit{dérivée faible} de
...
(La comparer à la première valeur propre de PDE-05(d).)
\item[(d)] Montrer que l'injection dans les
...
\textbf{PDE-16.} Le principe de Dirichlet affirme que la
...
\textbf{PDE-17.} Les valeurs propres de Dirichlet d'un domaine plan
...
\textbf{PDE-25.} Démontrer que pour toute fonction $g$ continue sur le
...
\textbf{PDE-26.} Pour l'équation de Schrödinger libre
...
\textbf{PDE-27.} On considère l'équation de Fisher--KPP sur la droite,
...
$(0,0)$ est un n\oe{}ud stable quand $c \ge 2$ et un foyer stable quand $0 < c < 2$.
...
\textbf{PDE-34.} Soit $\Phi$ la solution élémentaire du laplacien : une fonction radiale,
...
comment le transfert des dérivées sur $f$ répare l'argument --- la même man\oe{}uvre qu'exige la
...
\textbf{PDE-35.} Résoudre $u_{tt} = \Delta u$ avec les données $u(\cdot,0) = g$,
...
\textbf{PDE-36.} Pour $f$ continue, démontrer que le problème de Neumann
...
\textbf{PDE-37.} Énoncer le théorème de Cauchy--Kowalevski --- un problème de
...
\textbf{PDE-18.} Les équations de Navier--Stokes incompressibles
...
\textbf{PDE-19.} Supprimons la viscosité : les équations d'Euler
...
\textbf{PDE-20.} La théorie de la turbulence d'ondes prédit qu'une mer
...
\textbf{PDE-28.} Onsager a conjecturé en 1949 qu'une solution
...
\textbf{PDE-29.} La conjecture des points chauds de Rauch (1974) affirme
...
\textbf{PDE-30.} L'équation quasi-géostrophique de surface (SQG) pour un scalaire
...
\textbf{PDE-38.} On considère les solutions bornées de l'équation d'Allen--Cahn
...
\textbf{PDE-39.} Soit $(M,g)$ une variété riemannienne lisse fermée de dimension $n$ et
...
\textbf{PDE-40.} La conjecture de Schiffer affirme que si un domaine borné
